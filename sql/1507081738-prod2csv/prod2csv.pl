...
where:
    ...
    -D deliomiter - CSV delimiter characer. default is a tab.
...
######################################################################
#
# read and process product files, either CRB or MAI
#
sub read_file
{
...
{
    my ($prod_file, $pprod_db, $prod_dir, $section) = @_;
    #
    my $outnm = $prod_dir . '/' . $section . ".csv";
...
sub export_to_csv
{
    ...
    my $prod_dir = $csv_dir . '/CSV_' . $prod_name;
    #
    ( mkpath($prod_dir) || die $! ) unless ( -d $prod_dir );
    #
    printf $log_fh "\t\t%d: product %s CSV directory: %s\n", 
        __LINE__, $prod_name, $prod_dir;
    #
...
sub process_file
{
    ...
    {
        printf $log_fh "\t%d: ERROR Exporting product file: %s\n", 
                       __LINE__, $prod_file;
    }
    else
    {
