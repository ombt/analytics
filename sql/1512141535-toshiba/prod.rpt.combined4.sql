...
select
    ...
    -- p.place_count,
    p.boards_produced,
    case when p.boards_produced > 20 
    then
        'NOT_OK'
    else
        'OK'
...
on
    phr.end_time > ph.start_time
and
    phr.end_time <= ph.end_time + 2*60
and
    phr.equipment_id = ph.equipment_id
and
    phr.lane_no = ph.lane_no
and
    phr.setup_id = ph.setup_id
and
    phr.product_id = ph.product_id
left join
    production_reports_kx_raw pr
on
    pr.report_id = phr.report_id
and
    pr.stage_no = p.stage_no
where
    ph.equipment_id in ( 1001, 1003, 1005, 1009 )
-- and
    -- p.boards_produced > 50
