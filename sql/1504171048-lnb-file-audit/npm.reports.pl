        ...
        else
        {
            ...
            if (($blkserial eq $cache_blkserial) || ($is_tray == TRUE))
            {
                ...
                foreach my $col (@feeder_count_cols)
                {
                    ...
                    {
                         printf "%d WARNING: [%s] [%s] %s FAdd %s, FSAdd %s using NEGATIVE delta for key %s: %d\n", __LINE__, $filename, $cache_filename, $section, $fadd, $fsadd, $col, $delta;
                        $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col} = $delta;
                    }
                    else
                    {
                        $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col} = 0;
                         printf "%d WARNING: [%s] [%s] %s FAdd %s, FSAdd %s setting NEGATIVE delta (%d) for key %s to ZERO\n", __LINE__, $filename, $cache_filename, $section, $fadd, $fsadd, $delta, $col;
                    }
                }
            }
            else
