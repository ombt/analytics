...
sub calculate_u01_nozzle_delta
{
    ...
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        ...
        {
            printf "ERROR: [%s] %s NHAdd %s, NCAdd %s NOT found in cache. Taking all counts as is.\n",
...
sub tabulate_u01_nozzle_delta
{
    ...
    my $section = MOUNTPICKUPNOZZLE;
    #
    foreach my $machine (sort keys %{$pdb->{$section}})
    {
        foreach my $lane (sort keys %{$pdb->{$section}{$machine}})
        {
            foreach my $stage (sort keys %{$pdb->{$section}{$machine}{$lane}})
            {
                foreach my $nhadd (sort keys %{$pdb->{$section}{$machine}{$lane}{$stage}{delta}})
                {
                    foreach my $ncadd (sort keys %{$pdb->{$section}{$machine}{$lane}{$stage}{delta}{$nhadd}})
                    {
...
sub audit_u01_nozzles
{
    ...
    if ($verbose >= MAXVERBOSE)
    {
        ...
        printf "OUTPUT NO: %s\n", $output_no;
        printf "FILE RECS : %d\n", scalar(@{$pu01->{data}});
        printf "%s RECS: %d\n", $section, scalar(@{$pu01->{$section}->{data}});
    }
    #
...
sub print_u01_report
{
    ...
    #
    foreach my $machine (sort keys %{$totals{$section}{by_machine_lane}})
    {
        foreach my $lane (sort keys %{$totals{$section}{by_machine_lane}{$machine}})
        {
    ...
    #
    foreach my $machine (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}})
    {
        foreach my $lane (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}})
        {
            foreach my $stage (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}})
            {
                foreach my $nhadd (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}})
                {
                    foreach my $ncadd (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}{$nhadd}})
                    {
                        foreach my $blkserial (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}})
                        {
                            printf "Machine: %s, Lane: %s, Stage: %s, NHAdd: %s, NCAdd: %s, Blkserial: %s\n",
    ...
    #
    foreach my $machine (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd}})
    {
        foreach my $lane (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}})
        {
            foreach my $stage (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}})
            {
                foreach my $nhadd (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}})
                {
                    foreach my $ncadd (sort keys %{$totals{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}})
                    {
                        printf "Machine: %s, Lane: %s, Stage: %s, NHAdd: %s, NCAdd: %s\n", $machine, $lane, $stage, $nhadd, $ncadd;
                        foreach my $col (@nozzle_count_cols)
                        {
                            printf "\t%-${section_precision}s: %d\n", 
                                $col, 
