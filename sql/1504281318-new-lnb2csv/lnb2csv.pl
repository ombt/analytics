...
sub set_product_info
{
    ...
    #
    printf $log_fh "%d: Product %s: %s, Change Over: %d\n", __LINE__, $ftype, $pdb->{product}{$ftype}{$machine}{$lane}{$stage}, $pdb->{change_over}{u03}{$machine}{$lane}{$stage} if ($verbose >= MIDVERBOSE);
}
#
########################################################################
########################################################################
                ...
                foreach my $stage (sort { $a <=> $b } keys %{$totals{$section}{by_product}{$product}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}})
                {
                    foreach my $nhadd (sort { $a <=> $b } keys %{$totals{$section}{by_product}{$product}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}})
                    {
                        foreach my $ncadd (sort { $a <=> $b } keys %{$totals{$section}{by_product}{$product}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}})
                        {
                            ...
                            foreach my $col (@nozzle_count_cols)
                            {
                                printf $outfh ",%d", $totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$col};
                            }
                            printf $outfh "\n";
                        }
                    }
...
sub tabulate_u01_name_value_delta
{
    ...
    foreach my $key (keys %{$pu01->{$section}->{data}})
    {
        ...
        #
        if (exists($totals{$section}{by_product}{$product}{$section}{totals}{$key}))
        {
            $totals{$section}{by_product}{$product}{$section}{totals}{$key} += 
        ...
        printf $log_fh "%d: %s %s %s total=%d\n", __LINE__, $product, $section, $key, $totals{$section}{by_product}{$product}{$section}{totals}{$key} if ($verbose >= MAXVERBOSE);
        #
        if (exists($totals{$section}{by_product}{$product}{$section}{by_machine}{$machine}{$key}))
        {
            $totals{$section}{by_product}{$product}{$section}{by_machine}{$machine}{$key} += 
                $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$key};
        }
        else
        {
        ...
        printf $log_fh "%d: %s %s %s %s total=%d\n", __LINE__, $product, $section, $machine, $key, $totals{$section}{by_product}{$product}{$section}{by_machine}{$machine}{$key} if ($verbose >= MAXVERBOSE);
        #
        if (exists($totals{$section}{by_product}{$product}{$section}{by_machine_lane}{$machine}{$lane}{$key}))
        {
            $totals{$section}{by_product}{$product}{$section}{by_machine_lane}{$machine}{$lane}{$key} += 
                $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$key};
        }
        else
        {
        ...
        printf $log_fh "%d: %s %s %s %s %s total=%d\n", __LINE__, $product, $section, $machine, $lane, $key, $totals{$section}{by_product}{$product}{$section}{by_machine_lane}{$machine}{$lane}{$key} if ($verbose >= MAXVERBOSE);
        #
        if (exists($totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage}{$machine}{$lane}{$stage}{$key}))
        {
            $totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage}{$machine}{$lane}{$stage}{$key} += 
                $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$key};
        }
        else
        {
        ...
        printf $log_fh "%d: %s %s %s %s %s %s total=%d\n", __LINE__, $product, $section, $machine, $lane, $stage, $key, $totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage}{$machine}{$lane}{$stage}{$key} if ($verbose >= MAXVERBOSE);
        #
...
sub tabulate_u01_feeder_delta
{
    ...
    foreach my $fadd (sort { $a <=> $b } keys %{$pdb->{$section}{$machine}{$lane}{$stage}{delta}})
    {
        ...
        foreach my $fsadd (sort { $a <=> $b } keys %{$pdb->{$section}{$machine}{$lane}{$stage}{delta}{$fadd}})
        {
            ...
            # product-dependent totals
            #
            if (exists($totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage_fadd_fsadd_reelid}{$machine}{$lane}{$stage}{$fadd}{$fsadd}{$reelid}))
            {
                foreach my $col (@feeder_count_cols)
                {
                    $totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage_fadd_fsadd_reelid}{$machine}{$lane}{$stage}{$fadd}{$fsadd}{$reelid}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col};
                }
            }
            else
            {
            ...
            #
            if (exists($totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage_table_no}{$machine}{$lane}{$stage}{$table_no}))
            {
                foreach my $col (@feeder_count_cols)
                {
                    $totals{$section}{by_product}{$product}{$section}{by_machine_lane_stage_table_no}{$machine}{$lane}{$stage}{$table_no}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col};
                }
            }
            else
            {
...
sub audit_u01_feeders
{
    ...
    my $filename = $pu01->{file_name};
    #
    set_section_column_names(LNB_U01_FILE_TYPE, $pu01, $section);
    #
    printf $log_fh "\n%d: SECTION  : %s\n", __LINE__, $section
        if ($verbose >= MAXVERBOSE);
    #
...
sub audit_u01_files
{
    ...
    {
        printf $log_fh "\n%d: Audit U01: %s\n", $pu01->{file_name}, __LINE__ if ($verbose >= MIDVERBOSE);
        #
...
# process U03 files.
#
sub export_u03_section
{
    my ($pdb, $section, $export_type) = @_;
    #
    ...
    #
    open(my $outfh, ">" , "${export_dir}/${export_type}_BY_MACHINE_LANE_STAGE_FILENAME.csv") || die $!;
    #
    my $pcols = $totals{column_names}{LNB_U03_FILE_TYPE()}{$section};
    #
    printf $outfh "machine,lane,stage,filename";
    foreach my $col (@{$pcols})
    {
        printf $outfh ",%s", $col;
    }
    ...
    #
    open($outfh, ">" , "${export_dir}/${export_type}_BY_PRODUCT_MACHINE_LANE_STAGE_FILENAME.csv") || die $!;
    #
    printf $outfh "product,machine,lane,stage,filename";
    foreach my $col (@{$pcols})
    {
        printf $outfh ",%s", $col;
    }
...
sub export_u03_data
{
    my ($pdb) = @_;
    #
    export_u03_section($pdb, MOUNTQUALITYTRACE, 'MOUNT_QUALITY_TRACE');
    export_u03_section($pdb, MOUNTLATESTREEL, 'MOUNT_LATEST_REEL');
    export_u03_section($pdb, MOUNTEXCHANGEREEL, 'MOUNT_EXCHANGE_REEL');
}
#
sub tabulate_u03_section
{
...
sub audit_u03_section
{
    ...
    my $filename = $pu03->{file_name};
    #
    set_section_column_names(LNB_U03_FILE_TYPE, $pu03, $section);
    #
    printf $log_fh "\n%d: SECTION  : %s\n", __LINE__, $section 
        if ($verbose >= MAXVERBOSE);
    #
    if ($verbose >= MAXVERBOSE)
    {
        ...
        printf $log_fh "%d: STAGE    : %d\n", __LINE__, $stage;
        printf $log_fh "%d: OUTPUT NO: %s\n", __LINE__, $output_no;
        printf $log_fh "%d: FILE RECS : %d\n", __LINE__, scalar(@{$pu03->{data}});
        printf $log_fh "%d: %s RECS: %d\n", __LINE__, $section, scalar(@{$pu03->{$section}->{data}}) if (defined(@{$pu03->{$section}->{data}}));
    }
    #
    ...
    return unless (($output_no == PROD_COMPLETE) ||
                   ($output_no == PROD_COMPLETE_LATER));
    #
    tabulate_u03_section($pdb, $pu03, $section);
    #
    return;
}
#
sub audit_u03_file
{
    ...
    #
    audit_u03_section($pdb, $pu03, MOUNTQUALITYTRACE);
    audit_u03_section($pdb, $pu03, MOUNTLATESTREEL);
    audit_u03_section($pdb, $pu03, MOUNTEXCHANGEREEL);
    #
    return;
}
#
...
sub export_mpr_section
{
    ...
    open(my $outfh, ">" , "${export_dir}/${export_type}_BY_MACHINE_LANE_STAGE_FILENAME.csv") || die $!;
    #
    my $pcols = $totals{column_names}{LNB_MPR_FILE_TYPE()}{$section};
    #
    printf $outfh "sp,lane,stage,filename";
    foreach my $col (@{$pcols})
    {
        printf $outfh ",%s", $col;
    }
...
sub export_mpr_data
{
    my ($pdb) = @_;
    #
    export_mpr_section($pdb, TIMEDATASP, 'TIME_DATA_SP');
    export_mpr_section($pdb, COUNTDATASP, 'COUNT_DATA_SP');
    export_mpr_section($pdb, COUNTDATASP2, 'COUNT_DATA_SP2');
    export_mpr_section($pdb, TRACEDATASP, 'TRACE_DATA_SP');
    export_mpr_section($pdb, TRACEDATASP_2, 'TRACE_DATA_SP_2');
    export_mpr_section($pdb, ISPINFODATA, 'ISP_INFO_DATA');
    export_mpr_section($pdb, MASKISPINFODATA, 'MASK_ISP_INFO_DATA');
}
#
sub tabulate_mpr_section
{
    my ($pdb, $pmpr, $section) = @_;
    #
    my $filename = $pmpr->{file_name};
    my $machine = $pmpr->{mach_no};
    my $lane = $pmpr->{lane};
    my $stage = $pmpr->{stage};
    my $output_no = $pmpr->{output_no};
    #
    if ( ! exists($pmpr->{$section}))
    {
        printf $log_fh "%d: WARNING: Section %s does NOT exist in file %s\n", __LINE__, $section, $filename if ($verbose >= MINVERBOSE);
        return;
    }
    ...
    @{$totals{$section}{by_machine_lane_stage_filename}{$machine}{$lane}{$stage}{$filename}{data}} = ();
    #
    foreach my $prow (@{$pmpr->{$section}->{data}})
    {
        unshift @{$totals{$section}{by_machine_lane_stage_filename}{$machine}{$lane}{$stage}{$filename}{data}}, $prow;
    }
}
#
sub audit_mpr_section
{
    my ($pdb, $pmpr, $section) = @_;
    #
    my $machine = $pmpr->{mach_no};
    ...
    my $filename = $pmpr->{file_name};
    #
    set_section_column_names(LNB_MPR_FILE_TYPE(), $pmpr, $section);
    #
    printf $log_fh "\n%d: SECTION  : %s\n", __LINE__, $section
        if ($verbose >= MAXVERBOSE);
    #
    if ($verbose >= MAXVERBOSE)
    {
        ...
        printf $log_fh "%d: STAGE    : %d\n", __LINE__, $stage;
        printf $log_fh "%d: OUTPUT NO: %s\n", __LINE__, $output_no;
        printf $log_fh "%d: FILE RECS : %d\n", __LINE__, scalar(@{$pmpr->{data}});
        printf $log_fh "%d: %s RECS: %d\n", __LINE__, $section, scalar(@{$pmpr->{$section}->{data}}) if (defined(@{$pmpr->{$section}->{data}}));
    }
    #
    # check if the file has section.
    #
    return unless (($output_no == PROD_COMPLETE) ||
                   ($output_no == PROD_COMPLETE_LATER));
    #
    tabulate_mpr_section($pdb, $pmpr, $section);
    #
    return;
}
#
sub audit_mpr_file
{
    my ($pdb, $pmpr) = @_;
    #
    audit_mpr_section($pdb, $pmpr, TIMEDATASP());
    audit_mpr_section($pdb, $pmpr, COUNTDATASP());
    audit_mpr_section($pdb, $pmpr, COUNTDATASP2());
    audit_mpr_section($pdb, $pmpr, TRACEDATASP());
    audit_mpr_section($pdb, $pmpr, TRACEDATASP_2());
    audit_mpr_section($pdb, $pmpr, ISPINFODATA());
    audit_mpr_section($pdb, $pmpr, MASKISPINFODATA());
    #
    return;
}
#
