...
sub copy_u01_name_value_delta
{
    ...
    my $output_no = $pu01->{output_no};
    #
    $pcache->{$section}->{$machine}{$lane}{$stage}{delta} = undef;
    #
    foreach my $key (keys %{$pu01->{$section}->{data}})
    {
...
sub calculate_u01_nozzle_delta
{
    ...
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        ...
        #
        if ( ! exists($pcache->{$section}->{$machine}{$lane}{$stage}{data}{$nhadd}{$ncadd}{data}))
        {
            ...
            {
                $pcache->{$section}->{$machine}{$lane}{$stage}{delta}{$nhadd}{$ncadd}{data}{$col} = $prow->{$col};
            }
        }
        else
        {
            my $cache_blkserial = $pcache->{$section}->{$machine}{$lane}{$stage}{data}{$nhadd}{$ncadd}{data}{BLKSerial};
            if ($blkserial eq $cache_blkserial)
            {
                foreach my $col (@nozzle_count_cols)
                {
                    my $u01_value = $prow->{$col};
                    my $cache_value = $pcache->{$section}->{$machine}{$lane}{$stage}{data}{$nhadd}{$ncadd}{data}{$col};
                }

                $pcache->{$section}->{$machine}{$lane}{$stage}{delta}{$nhadd}{$ncadd}{delta}{Pickup} = $prow->{
                $pcache->{$section}->{$machine}{$lane}{$stage}{delta}{$nhadd}{$ncadd}{delta}{Pickup} = 
            }
            else
            {
            }
...
sub copy_u01_nozzle_cache
{
    ...
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        ...
        foreach my $col (@{$pcols})
        {
            $pcache->{$section}->{$machine}{$lane}{$stage}{data}{$nhadd}{$ncadd}{data}{$col} = $prow->{$col};
        }
        #
        $pcache->{$section}->{$machine}{$lane}{$stage}{data}{$nhadd}{$ncadd}{state} = $state;
    }
}
#
...
sub audit_u01_nozzles
{
           ...
           ($output_no == PROD_COMPLETE_LATER))
    {
        if ( ! exists($pcache->{$section}->{$machine}{$lane}{$stage}{state}))
        {
            ...
            $pcache->{$section}->{$machine}{$lane}{$stage}{data} = undef;
            #
            copy_u01_nozzle_cache($pcache, $pu01, DELTA);
        }
        elsif ($pcache->{$section}->{$machine}{$lane}{$stage}{state} eq RESET)
        {
            printf "ENTRY STATE: %s\n", 
                $pcache->{$section}->{$machine}{$lane}{$stage}{state}
                if ($verbose >= MAXVERBOSE);
            copy_u01_nozzle_delta($pcache, $pu01);
            tabulate_u01_nozzle_delta($pcache, $pu01);
            copy_u01_nozzle_cache($pcache, $pu01, DELTA);
            #
            $pcache->{$section}->{$machine}{$lane}{$stage}{state} = DELTA;
        }
        elsif ($pcache->{$section}->{$machine}{$lane}{$stage}{state} eq DELTA)
        {
            printf "ENTRY STATE: %s\n", 
                $pcache->{$section}->{$machine}{$lane}{$stage}{state}
                if ($verbose >= MAXVERBOSE);
            calculate_u01_nozzle_delta($pcache, $pu01);
            tabulate_u01_nozzle_delta($pcache, $pu01);
            copy_u01_nozzle_cache($pcache, $pu01, DELTA);
            #
            $pcache->{$section}->{$machine}{$lane}{$stage}{state} = DELTA;
