...
sub calculate_u01_nozzle_delta
{
    ...
    my $pcols = $pu01->{$section}->{column_names};
    #
    delete $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta};
    #
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        my $nhadd = $prow->{NHAdd};
        ...
        #
        if ( ! exists($pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_cache}{$nhadd}{$ncadd}{data}))
        {
            ...
            foreach my $col (@{$pcols})
            {
                $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col} = $prow->{$col};
            }
        }
        else
        {
            my $cache_blkserial = $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_cache}{$nhadd}{$ncadd}{data}{BLKSerial};
            if ($blkserial eq $cache_blkserial)
            {
                $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{BLKSerial} = $blkserial;
                #
                foreach my $col (@nozzle_count_cols)
                {
                    my $u01_value = $prow->{$col};
                    my $cache_value = $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_cache}{$nhadd}{$ncadd}{data}{$col};
                    #
                    my $delta = $u01_value - $cache_value;
                    #
                    if ($delta >= 0)
                    {
                        $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col} = $delta;
                    }
                    elsif (($proc_options & PROC_OPT_USENEGDELTS) != 0)
                    {
                    ...
                    {
                        $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col} = 0;
            ...
            else
            {
                ...
                #
                delete $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data};
                #
                foreach my $col (@{$pcols})
                {
                    $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col} = $prow->{$col};
...
sub copy_u01_nozzle_cache
{
    ...
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        ...
        foreach my $col (@{$pcols})
        {
            $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_cache}{$nhadd}{$ncadd}{data}{$col} = $prow->{$col};
        }
        #
        $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_cache}{$nhadd}{$ncadd}{state} = $state;
    }
}
#
...
sub tabulate_u01_nozzle_delta
{
    ...
    my $product = $pdb->{product}{u01}{$machine}{$lane}{$stage};
    #
    foreach my $nhadd (sort { $a <=> $b } keys %{$pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}})
    {
        foreach my $ncadd (sort { $a <=> $b } keys %{$pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}})
        {
            my $blkserial = $pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{BLKSerial};
            #
            # product-independent totals
            #
            if (exists($totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$blkserial}))
            {
                foreach my $col (@nozzle_count_cols)
                {
                    $totals{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$blkserial}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col};
                }
            }
            else
            {
            ...
            if (exists($totals{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}))
            {
                foreach my $col (@nozzle_count_cols)
                {
                    $totals{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col};
                }
            }
            else
            {
            ...
            if (exists($totals{by_product}{$product}{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$blkserial}))
            {
                foreach my $col (@nozzle_count_cols)
                {
                    $totals{by_product}{$product}{$section}{by_machine_lane_stage_nhadd_ncadd_blkserial}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$blkserial}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col};
                }
            }
            else
            {
            ...
            if (exists($totals{by_product}{$product}{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}))
            {
                foreach my $col (@nozzle_count_cols)
                {
                    $totals{by_product}{$product}{$section}{by_machine_lane_stage_nhadd_ncadd}{$machine}{$lane}{$stage}{$nhadd}{$ncadd}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{nh_nc_delta}{$nhadd}{$ncadd}{data}{$col};
                }
            }
            else
            {
           ...
           ($output_no == PROD_COMPLETE_LATER))
    {
        if ( ! exists($pdb->{$section}->{$machine}{$lane}{$stage}{state}))
        {
            printf $log_fh "ENTRY STATE: UNKNOWN\n", 
                if ($verbose >= MAXVERBOSE);
            #
            delete $pdb->{$section}->{$machine}{$lane}{$stage}{nh_nc_cache};
