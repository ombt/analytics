...
echo "$may_start $may_end $may_trace_data_sql" >> ${all_out}
#
cat ${trace_data_sql} |
...
echo "$today_start $today_end $today_trace_data_sql" >> ${all_out}
#
cat ${trace_data_sql} |
...
inner join
    equipment eq
on
    eq.equipment_id = prh.equipment_id
order by
...
EOF
#
select_product_run_history_sql="/tmp/$$.sprh.sql"
echo "$select_product_run_history_sql"
#
may_product_run_history_sql="/tmp/$$.mprh.sql"
echo "$may_product_run_history_sql"
#
today_product_run_history_sql="/tmp/$$.tprh.sql"
echo "$today_product_run_history_sql"
#
cat > ${select_product_run_history_sql} <<EOF
select 
    prh.product_run_history_id as 'prh.product_run_history_id',
    prh.equipment_id as 'prh.equipment_id',
    eq.equipment_name as 'eq.equipment_name',
...
on
    eq.equipment_id = prh.equipment_id
where
    prh.end_time >= START_TIME
and
    END_TIME >= prh.start_time
order by
    prh.equipment_id asc,
    prh.lane_no asc,
    prh.start_time asc
...
EOF
#
cat ${select_product_run_history_sql} |
sed -e "s/START_TIME/${may_start}/" \
    -e "s/END_TIME/${may_end}/" > ${may_product_run_history_sql}
#
cat ${select_product_run_history_sql} |
sed -e "s/START_TIME/${today_start}/" \
    -e "s/END_TIME/${today_end}/" > ${today_product_run_history_sql}
#
#######################################################################
#
# start of script
...
#
may_product_run_history_sql_out="${may_product_run_history_sql}.out"
time my_bsql ${may_product_run_history_sql} ${may_product_run_history_sql_out}
#
today_product_run_history_sql_out="${today_product_run_history_sql}.out"
time my_bsql ${today_product_run_history_sql} ${today_product_run_history_sql_out}
#
cat ${may_trace_data_sql} \
    ...
    ${may_product_run_history_sql} \
    ${may_product_run_history_sql_out} \
    ${today_product_run_history_sql} \
