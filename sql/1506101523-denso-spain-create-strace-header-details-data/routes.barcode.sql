...
select
    ...
    mstp.stage_no,
    mstp.display_z_pu,
    mstp.display_table_slot,
    mstp.display_subslot
from
    routes r
inner join
    route_layout rl
...
inner join
    feeder_history fh
on
    zl.equipment_id = fh.equipment_id
inner join
    reel_data rd
on
    rd.reel_id = fh.reel_id
inner join
    machine_slot_to_pu mstp
on
    mstp.equipment_id = fh.equipment_id
and
    mstp.slot = fh.slot
and
    mstp.subslot = fh.subslot
where
    r.valid_flag = 'T'
and
    z.module_id <> -1
...
and
    fh.time_on <= pbm.timestamp
and
    pbm.timestamp < fh.time_off
and
    pbm.timestamp in
    ( 
--         1431105684,
