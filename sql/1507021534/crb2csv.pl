...
sub load_name_value
{
    ...
    my $start_irec = $$pirec;
    #
    for ( ; $$pirec<$max_rec; $$pirec++)
    {
        my $rec = $praw_data->[$$pirec];
        #
        last if ($rec =~ m/^\s*$/);
    }
    #
...
sub load_list
{
    ...
    {
        printf $log_fh "\t\t\t%d: Empty list section\n", __LINE__;
    }
}
#
######################################################################
...
sub process_data
{
    ...
    my $sec_no = 0;
    #
    for (my $irec=0; $irec<$max_rec; $irec++)
    {
...
sub process_file
{
    ...
    #
    my @raw_data = ();
    return FAIL unless (read_file($crb_file, \@raw_data) == SUCCESS);
    #
    my %crb_db = ();
    return FAIL unless (process_data($crb_file, \@raw_data, \%crb_db) == SUCCESS);
    #
    return FAIL unless (export_to_csv($crb_file, \%crb_db) == SUCCESS);
    #
    return SUCCESS;
}
#
