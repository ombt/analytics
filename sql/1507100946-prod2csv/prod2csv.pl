...
use constant FILE_TYPE_CRB => 1;
use constant FILE_TYPE_MAI => 2;
#
my %file_types =
(
...
where:
    ...
    -d path - csv directory, defaults to './CSV'.
...
sub export_to_csv
{
    ...
    #
    rmtree($prod_dir) if ($rmv_prod_dir == TRUE);
...
#
my %opts;
if (getopts('?hwWv:d:l:D:R', \%opts) != 1)
{
    usage($cmd);
    exit 2;
}
