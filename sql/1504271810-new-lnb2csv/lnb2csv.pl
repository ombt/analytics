...
sub set_section_column_names
{
    ...
    {
        printf "No column data for %s %s.\n", $file_type, $section if ($verbose >= MAXVERBOSE);
    }
    elsif ( ! exists($totals{column_names}{$file_type}{$section}) )
    {
        ...
        #
        printf "\nSetting column names %s %s: %s\n", $file_type, $section, join(' ', @{$totals{column_names}{$file_type}{$section}});
    }
}
#
########################################################################
...
sub set_product_info
{
    ...
    #
    printf $log_fh "Product %s: %s, Change Over: %d\n", $ftype, $pdb->{product}{$ftype}{$machine}{$lane}{$stage}, $pdb->{change_over}{u03}{$machine}{$lane}{$stage} if ($verbose >= MIDVERBOSE);
}
#
########################################################################
########################################################################
...
sub export_u03_section
{
    ...
    ###############################################################
    #
    printf $log_fh "\nExport Data For %s by Machine, Lane, Stage, Filename:\n", $section;
    #
    ...
    ###############################################################
    #
    printf $log_fh "\nExport Data For %s by Product, Machine, Lane, Stage, Filename:\n", $section;
    #
    open(my $outfh, ">" , "${export_dir}/${export_type}_BY_PRODUCT_MACHINE_LANE_STAGE_FILENAME.csv") || die $!;
    #
...
sub tabulate_u03_section
{
    ...
    {
        printf $log_fh "WARNING: Section %s does NOT exist in file %s\n", $section, $filename if ($verbose >= MINVERBOSE);
        return;
    }
    #
