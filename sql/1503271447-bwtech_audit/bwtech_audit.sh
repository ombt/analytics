...
# sql commands
#
Z_CASS_HDR_RAW_SQL="/tmp/$$.zchr.sql"
#
cat > ${Z_CASS_HDR_RAW_SQL} <<EOF
select
...
#
echo -e "\nGetting Z_CASS_HEADER_RAW DB Data From ${MINSQLTIME} to ${MAXSQLTIME}"
#
TMPSQL="/tmp/sql.$$"
cat ${Z_CASS_HDR_RAW_SQL} |
sed -e "s/SQL_MIN_TIME/${MINSQLTIME}/g" \
    -e "s/SQL_MAX_TIME/${MAXSQLTIME}/g" > ${TMPSQL}
Z_CASS_HDR_RAW_SQL_OUT="${Z_CASS_HDR_RAW_SQL}.out"
time my_bsql ${TMPSQL} ${Z_CASS_HDR_RAW_SQL_OUT}
#
...
#
# compare the DB data to the data in the PT log files.
#
for MACHINE in AB21DTP1_CM402 AB21DTP2_CM402
do
    echo -e "\nCompare DB Data to PT Log file for Machine: ${MACHINE}"
    #
    time bwtech_audit.pl ${MACHINE} ${Z_CASS_HDR_RAW_SQL_OUT} ${Z_CASS_RAW_SQL_OUT} 
done
#
exit 0
