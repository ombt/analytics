...
######################################################################
#
# process CRB file and create CSV files for each section.
#
######################################################################
#
use strict;
...
where:
    ...
    -d path - csv directory, defaults to './'.
...
sub load_name_value
{
    my ($praw_data, $section, $pirec, $max_rec, $pcrb_db) = @_;
    #
    $pcrb_db->{found_data}->{$section} = FALSE;
    #
    ...
    #
    %{$pcrb_db->{$section}->{data}} = 
        map { split /\s*=\s*/, $_, 2 } @section_data;
    #
    $pcrb_db->{found_data}->{$section} = TRUE;
    #
    printf $log_fh "\t\t\t%d: Number of key-value pairs: %d\n", 
        __LINE__, 
        scalar(keys %{$pcrb_db->{$section}->{data}})
...
sub load_list
{
    ...
    pop @section_data;   # remove end-of-section null-length line
    #
    $pcrb_db->{$section}->{header} = shift @section_data;
    @{$pcrb_db->{$section}->{column_names}} = 
        split / /, $pcrb_db->{$section}->{header};
    my $number_columns = scalar(@{$pcrb_db->{$section}->{column_names}});
    #
    @{$pcrb_db->{$section}->{data}} = ();
    #
    ...
    foreach my $record (@section_data)
    {
        ...
        #
        print $log_fh "\t\t\t%d: Number of tokens in record: %d\n", __LINE__, $number_tokens if ($verbose >= MAXVERBOSE);
        #
        if ($number_tokens == $number_columns)
        {
            my %data = ();
            @data{@{$pcrb_db->{$section}->{column_names}}} = @tokens;
            #
            unshift @{$pcrb_db->{$section}->{data}}, \%data;
            printf $log_fh "\t\t\t%d: Current Number of Records: %d\n", __LINE__, scalar(@{$pcrb_db->{$section}->{data}}) if ($verbose >= MAXVERBOSE);
        }
        else
        {
...
######################################################################
#
# read and process CRB files
#
sub read_file
{
    my ($crb_file, $praw_data) = @_;
    #
    printf $log_fh "\t%d: Reading CRB file: %s\n", 
                   __LINE__, $crb_file;
    #
    if ( ! -r $crb_file )
    {
        printf $log_fh "\t%d: ERROR: file $crb_file is NOT readable\n\n", __LINE__;
        return FAIL;
    }
    #
    unless (open(INFD, $crb_file))
    {
        printf $log_fh "\t%d: ERROR: unable to open $crb_file.\n\n", __LINE__;
        return FAIL;
    }
    @{$praw_data} = <INFD>;
    close(INFD);
...
sub process_data
{
    my ($crb_file, $praw_data, $pcrb_db) = @_;
    #
    printf $log_fh "\t%d: Processing CRB data: %s\n", 
                   __LINE__, $crb_file;
    #
...
sub export_to_csv
{
    my ($crb_file, $pcrb_db) = @_;
    #
    printf $log_fh "\t%d: Writing CRB data to CSV: %s\n", 
                   __LINE__, $crb_file;
    #
    #
    return SUCCESS;
...
sub process_file
{
    ...
    #
    my $status = FAIL;
    if (read_file($crb_file, \@raw_data) != SUCCESS)
    {
        printf $log_fh "\t%d: ERROR Reading CRB file: %s\n", 
                       __LINE__, $crb_file;
    }
    elsif (process_data($crb_file, \@raw_data, \%crb_db) != SUCCESS)
    {
        printf $log_fh "\t%d: ERROR Processing CRB file: %s\n", 
                       __LINE__, $crb_file;
    }
    elsif (export_to_csv($crb_file, \%crb_db) != SUCCESS)
    {
        printf $log_fh "\t%d: ERROR Exporting CRB file: %s\n", 
                       __LINE__, $crb_file;
    }
    else
    {
        printf $log_fh "\t%d: Success processing CRB file: %s\n", 
                       __LINE__, $crb_file;
        $status = SUCCESS;
    }
    #
...
foreach my $opt (%opts)
{
    ...
    {
        $csv_dir = $opts{$opt};
        mkpath($csv_dir) unless ( -d $csv_dir );
        printf $log_fh "\n%d: CSV directory: %s\n", __LINE__, $csv_dir;
    }
}
#
if (scalar(@ARGV) == 0)
{
    printf $log_fh "%d: No CRB files given.\n", __LINE__;
    usage($cmd);
    exit 2;
}
#
foreach my $crb_file (@ARGV)
{
    process_file($crb_file);
}
#
exit 0;
