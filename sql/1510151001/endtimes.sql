...
select
    hr.equipment_id,
    min(hr.end_time) as pr_min_etime,
    max(hr.end_time) as pr_max_etime
from
    production_reports_npm_hdr_raw hr
where
    hr.trx_productid like '20150923%'
group by
    hr.equipment_id
order by
    hr.equipment_id asc
...
select
    hr.equipment_id,
    min(hr.end_time) as nz_min_etime,
    max(hr.end_time) as nz_max_etime
from
    nozzle_npm_hdr_raw hr
where
    hr.trx_productid like '20150923%'
group by
    hr.equipment_id
...
select
    hr.equipment_id,
    min(hr.end_time) as zc_min_etime,
    max(hr.end_time) as zc_max_etime
from
    z_cass_npm_hdr_raw hr
where
    hr.trx_productid like '20150923%'
group by
    hr.equipment_id
