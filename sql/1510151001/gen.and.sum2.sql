...
print N'PROD_NPM SUMMED DELTA DATA ...';
go
...
select
    ...
    sum(prd.tpickup) as sum_prd_tpickup,
    sum(prd.tpmiss) as sum_prd_tpmiss,
    sum(prd.trmiss) as sum_prd_trmiss,
    sum(prd.tdmiss) as sum_prd_tdmiss,
    sum(prd.tmmiss) as sum_prd_tmmiss,
    sum(prd.thmiss) as sum_prd_thmiss
from
...
where
    prh.end_time >= 1442941200
and
    prh.end_time <= 1443027600
group by
    prh.product_id,
    prh.equipment_id,
    -- prh.setup_id,
...
print N'Z_CASS_NPM SUMMED DELTA DATA ...';
go
...
print N'PROD_NPM_RAW_CTE SUMMED GENERATED DELTA DATA ...';
go
...
print N'Z_CASS_NPM_RAW_CTE SUMMED GENERATED DELTA DATA ...';
go
...
select
    ...
    sum(delta_pkup) as sum_delta_pkup,
    sum(delta_plc) as sum_delta_plc,
    sum(delta_tmiss) as sum_delta_tmiss,
    sum(delta_rmiss) as sum_delta_rmiss,
    sum(delta_hmiss) as sum_delta_hmiss,
    sum(delta_fmiss) as sum_delta_fmiss,
    sum(delta_mmiss) as sum_delta_mmiss
from
(
    select
        -- curr.row as curr_row,
...
union
    select
        ...
        pkup as delta_pkup,
        plc as delta_plc,
        tmiss as delta_tmiss,
        rmiss as delta_rmiss,
        hmiss as delta_hmiss,
        fmiss as delta_fmiss,
        prev.mmiss as delta_mmiss
    from
        z_cass_cte 
    where
        row = 1
