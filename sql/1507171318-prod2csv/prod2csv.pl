...
sub export_list_to_db
{
        ...
        ($section !~ m/<([0-9]+)>/))
    {
        ...
        else
        {
            ...
            my $create_tbl_sql = "create table ${table_name} ( ";
            #
            my $pcols = $pprod_db->{$section}->{column_names};
            my $comma = "";
            foreach my $col (@{$pcols})
            {
                $create_tbl_sql .= "${comma} '${col}' " . 'varchar(100)';
                $comma = ',';
            }
            $create_tbl_sql .= ' )';
            #
        ...
        #
        my $insert_fields = "insert into ${table_name} ( ";
        my $insert_values = "values ( ";
        #
        my $pcols = $pprod_db->{$section}->{column_names};
        my $comma = "";
        foreach my $col (@{$pcols})
        {
            $insert_fields .= "${comma} '${col}' ";
            $insert_values .= "${comma} \? ";
            $comma = ",";
        }
        $insert_fields .= " ) ";
        $insert_values .= " ) ";
        #
        my $insert_sql = $insert_fields . $insert_values;
        #
        foreach my $prow (@{$pprod_db->{$section}->{data}})
        {
            $dbh->do($insert_sql, undef, $prow->{@{$pcols}});
        }
    }
    else
    {
