...
sub export_u01_count_report
{
    ...
    my $first_time = TRUE;
    #
    open(my $outfh, ">" , "${export_dir}/COUNT_BY_MACHINE.csv") || die $!;
    foreach my $machine (sort { $a <=> $b } keys %{$totals{$section}{by_machine}})
    {
        if ($first_time == TRUE)
        {
    ...
    foreach my $machine (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane}})
    {
        foreach my $lane (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane}{$machine}{$lane}})
        {
            if ($first_time == TRUE)
            {
                printf $outfh "machine,lane";
                foreach my $key (sort keys %{$totals{$section}{by_machine_lane}{$machine}})
                {
                    printf $outfh ",%s", $key;
                }
                printf $outfh "\n";
        ...
        foreach my $machine (sort { $a <=> $b } keys %{$totals{by_product}{$product}{$section}{by_machine_lane}})
        {
            foreach my $lane (sort { $a <=> $b } keys %{$totals{by_product}{$product}{$section}{by_machine_lane}{$machine}})
            {
                if ($first_time == TRUE)
                {
                    ...
                    foreach my $key (sort keys %{$totals{by_product}{$product}{$section}{by_machine}{$machine}{$lane}{$lane}})
                    {
                        printf $outfh ",%s", $key;
                    }
                    printf $outfh "\n";
...
#
my %opts;
if (getopts('?anxhvV:d:t:r:', \%opts) != 1)
{
    usage($cmd);
    exit 2;
}
