...
sub calculate_u01_feeder_delta
{
    ...
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        ...
        {
            printf "WARNING: [%s] %s FAdd %s, FSAdd %s NOT found in cache. Taking all counts as is.\n",
                $filename, $section, $fadd, $fsadd;
        ...
        {
            my $cache_blkserial = $pdb->{$section}->{$machine}{$lane}{$stage}{cache}{$fadd}{$fsadd}{data}{BLKSerial};
            if ($blkserial eq $cache_blkserial)
            {
                $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{BLKSerial} = $blkserial;
                ...
                foreach my $col (@feeder_count_cols)
                {
                    ...
                    {
                         printf "WARNING: [%s] %s FAdd %s, FSAdd %s using NEGATIVE delta for key %s: %d\n", $filename, $section, $fadd, $fsadd, $col, $delta;
                        $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col} = $delta;
                    }
                    else
                    {
                        $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col} = 0;
                         printf "WARNING: [%s] %s FAdd %s, FSAdd %s setting NEGATIVE delta (%d) for key %s to ZERO\n", $filename, $section, $fadd, $fsadd, $delta, $col;
                    }
                }
            }
            else
            {
                printf "WARNING: [%s] %s FAdd %s, FSAdd %s BLKSERIAL CHANGED: CACHED %s, CURRENT U01 %s\n", $cache_blkserial, $blkserial;
                #
...
sub tabulate_u01_feeder_delta
{
    ...
    foreach my $fadd (sort { $a <=> $b } keys %{$pdb->{$section}{$machine}{$lane}{$stage}{delta}})
    {
        ...
        {
            my $blkserial = $pdb->{$section}{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{BLKSerial};
            my $reelid = $pdb->{$section}{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{ReelID};
            #
            if (exists($totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}{$fadd}{$fsadd}{$blkserial}))
            {
                foreach my $col (@nozzle_count_cols)
                {
                    $totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}{$fadd}{$fsadd}{$blkserial}{$col} += $pdb->{$section}{$machine}{$lane}{$stage}{delta}{$fadd}{$fsadd}{data}{$col};
                }
            }
            else
            {
...
sub calculate_u01_nozzle_delta
{
    ...
    foreach my $prow (@{$pu01->{$section}->{data}})
    {
        ...
        {
            printf "WARNING: [%s] %s NHAdd %s, NCAdd %s NOT found in cache. Taking all counts as is.\n",
                $filename, $section, $nhadd, $ncadd;
            foreach my $col (@{$pcols})
            {
        ...
        else
        {
            ...
            if ($blkserial eq $cache_blkserial)
            {
                ...
                foreach my $col (@nozzle_count_cols)
                {
                    ...
                    {
                         printf "WARNING: [%s] %s NHAdd %s, NCAdd %s using NEGATIVE delta for key %s: %d\n", $filename, $section, $nhadd, $ncadd, $col, $delta;
                        $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$nhadd}{$ncadd}{data}{$col} = $delta;
                    }
                    else
                    {
                        $pdb->{$section}->{$machine}{$lane}{$stage}{delta}{$nhadd}{$ncadd}{data}{$col} = 0;
                         printf "WARNING: [%s] %s NHAdd %s, NCAdd %s setting NEGATIVE delta (%d) for key %s to ZERO\n", $filename, $section, $nhadd, $ncadd, $delta, $col;
                    }
                }
            }
            else
            {
                printf "WARNING: [%s] %s NHAdd %s, NCAdd %s BLKSERIAL CHANGED: CACHED %s, CURRENT U01 %s\n", $cache_blkserial, $blkserial;
                #
...
sub print_u01_feeder_report
{
    ...
    my $section_precision = $report_precision{$section}{precision};
    #
    printf "\nData For %s by Machine, Lane, Stage, FAdd, FSAdd, Blkserial:\n", $section;
    #
    ...
    printf "\n";
    #
    foreach my $machine (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}})
    {
        foreach my $lane (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}})
        {
            foreach my $stage (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}})
            {
                foreach my $fadd (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}})
                {
                    foreach my $fsadd (sort { $a <=> $b } keys %{$totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}{$fadd}})
                    {
                        foreach my $blkserial (sort keys %{$totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}{$fadd}{$fsadd}})
                        {
                            # my $reelid = $totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}{$fadd}{$fsadd}{$blkserial}{ReelID};
                            # printf "%-8s %-8s %-8s %-8s %-8s %-30s %-30s ",
                            printf "%-8s %-8s %-8s %-8s %-8s %-30s ",
                                $machine, $lane, $stage, $fadd, $fsadd, $blkserial;
                            foreach my $col (@feeder_count_cols)
                            {
                                printf "%-8d ", 
                                    $totals{$section}{by_machine_lane_stage_fadd_fsadd_blkserial}{$machine}{$lane}{$stage}{$fadd}{$fsadd}{$blkserial}{$col};
                            }
                            printf "\n";
                        }
                    }
