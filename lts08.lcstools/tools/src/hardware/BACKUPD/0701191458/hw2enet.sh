#!/usr/bin/ksh
#
# read tl1 hardware scripts and generates enet relation.
#
###########################################################################
#
usage="
...
do
	echo "FILE: $file"
	cat $file | 
	egrep -i '(ent-enet:|ent-enet-endptvoip:)' | 
	sed 's/^[	 ]*"//; s/"[	 ]*$//;' |
	tr '[A-Z]' '[a-z]'
done |
sed 's/^[ 	]*//g' |
...
$0 ~ /^ent-enet-endptvoip:/ {
	if (verbose) print "ENT-ENET-ENDPTVOIP: " $0;
	#
	colonno = split($0, colonfields, ":");
	for (ic=1; verbose && ic<=colonno; ic++)
	{
