...
usage: ${CMD} [-?] [-x] [-a|-C] [-o outfile] [-u username] [-p passwd]
...
where:
	...
	-C - run for all labs and branches in chassis relation
...
cd ${LCSTOOLSDATA}
#
if [[ "${alllabids}" == yes || "${usechassis}" == yes ]]
then
...
do
	...
	#
	echo
	echo "Using branch(es) for ${labid}: ${branches}."
	#
