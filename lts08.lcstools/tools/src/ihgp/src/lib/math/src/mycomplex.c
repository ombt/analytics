...
template <class T>
Complex<T>
pow(const Complex<T> &b, Complex<T> &p)
{
        if (b == 0.0)
...
{
        return(Complex<T>(cos(c.x)*cosh(c.y), -1.0*sin(c.x)*sinh(c.y)));
}
...
{
        return(Complex<T>(sinh(c.x)*cos(c.x), cosh(c.x)*sin(c.y)));
}
...
{
        return(Complex<T>(cosh(c.x)*cos(c.x), sinh(c.x)*sin(c.y)));
}
...
{
        T divider = cosh(2.0*c.x)*cos(2.0*c.y);
        return(Complex<T>(sinh(2.0*c.x)/divider, cos(2.0*c.y)/divider));
}
...
template <class T>
ostream &
operator<<(ostream &os, const Complex<T> &c)
{
        os << "(" << c.x << "," << c.y << ")";
        return(os);
}
