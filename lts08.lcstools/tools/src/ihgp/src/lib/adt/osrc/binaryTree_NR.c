...
template <class DataType>
int
BinaryTree_NR<DataType>::isInTree(const DataType &data) const
{
	// find data, if any.
	BinaryTreeNode_NR<DataType> *pn = root;
	while (pn != NULL)
...
BinaryTree_NR<DataType>::clear()
{
	...
	pq = new Queue_List<BinaryTreeNode_NR<DataType> * >;
	MustBeTrue(pq != NULL);
...
BinaryTree_NR<DataType>::preOrder(int (*work)(DataType &))
{
	...
	ps = new Stack_List<BinaryTreeNode_NR<DataType> * >;
	MustBeTrue(ps != NULL);
...
template <class DataType>
ostream &
BinaryTree_NR<DataType>::dump(ostream &os) const
{
	// check if tree is empty
	if (root == NULL)
		return(os);
...
BinaryTree_NR<DataType>::copy() const
{
	...
	ps1 = new Stack_List<BinaryTreeNode_NR<DataType> * >;
	MustBeTrue(ps1 != NULL);
	Stack_List<BinaryTreeNode_NR<DataType> ** > *ps2;
	ps2 = new Stack_List<BinaryTreeNode_NR<DataType> ** >;
	MustBeTrue(ps2 != NULL);
...
{
	stack.clear();
	for (BinaryTreeNode_NR<DataType> *pn = tree->root; 
		pn != NULL; pn = pn->left)
	{
		stack.push(pn);
	}
