...
template <class DataType>
int
BinaryTree_Array<DataType>::isInTree(const DataType &data) const
{
	// find data, if any.
	int n = root;
	while (n != -1)
...
BinaryTree_Array<DataType>::preOrder(int (*work)(DataType &))
{
	...
	Stack_List<int> *ps = new Stack_List<int>;
	MustBeTrue(ps != NULL);
...
BinaryTree_Array<DataType>::postOrder(int (*work)(DataType &))
{
	...
	int visits;
	Stack_List<int> *pvs = new Stack_List<int>;
	MustBeTrue(pvs != NULL);
	Stack_List<int> *ps = new Stack_List<int>;
	MustBeTrue(ps != NULL);
...
BinaryTree_Array<DataType>::levelOrder(int (*work)(DataType &))
{
	...
	Queue_List<int> *pq = new Queue_List<int>;
	MustBeTrue(pq != NULL);
...
template <class DataType>
ostream &
BinaryTree_Array<DataType>::dump(ostream &os) const
{
	// check if tree is empty
	if (root == -1)
		return(os);
...
{
	stack.clear();
	for (int pn = tree->root; pn != -1; pn = tree->array[pn].left)
	{
		stack.push(pn);
	}
	return;
...
BinaryTree_Array_Iterator_InOrder<DataType>::operator++(int)
{
	...
	if (tree->array[btn].right != -1)
	{
		for (int pn = tree->array[btn].right;
			pn != -1; pn = tree->array[pn].left)
		{
			stack.push(pn);
		}
	}
