...
HashTable_List<DataType>::remove(DataType &d)
{
	...
	// remove from bucket
	return(array[ib].removeByValue(d));
...
HashTable_List<DataType>::retrieve(DataType &d) const
{
	...
	return(array[ib].retrieveByValue(d));
}

template <class DataType>
int
HashTable_List<DataType>::isInTable(const DataType &d) const
{
	// get bucket for data item
	int ib = hash(d)%size;

	// check if in bucket
	return(array[ib].isInList(d));
}
...
template <class DataType>
ostream &
HashTable_List<DataType>::dump(ostream &os) const
{
	os << "hash table = {" << endl;
	for (int ib = 0; ib < size; ib++)
	{
	        os << "bucket[" << ib << "] = " << array[ib] << endl;
	}
	os << "}" << endl;
	return(os);
}
