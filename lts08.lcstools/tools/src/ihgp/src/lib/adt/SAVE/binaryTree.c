...
BinaryTree<DataType>::insert(const DataType &data)
{
	...
	if ( ! iterList.isEmpty())
	{
		...
		ListIterator<AbstractIterator<DataType> * > *
			piter = new ListIterator<AbstractIterator<DataType> * >(iterList);
...
template <class DataType>
int
BinaryTree<DataType>::isInTree(const DataType &data) const
{
...
BinaryTree<DataType>::levelOrder(int (*work)(DataType &))
{
	...
	pq = new Queue_List<BinaryTreeNode<DataType> * >();
	MustBeTrue(pq != NULL);
			     ...
			     const DataType &data)
{
	...
	{
		insert(btn->left, data);
	}
	else if (data > btn->data)
	{
		insert(btn->right, data);
	}
	else
	{
		btn->data = data;
...
template <class DataType>
int
BinaryTree<DataType>::isInTree(const BinaryTreeNode<DataType> *node, 
