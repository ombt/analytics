...
main(int argc, char **argv)
{
	...
	MustBeTrue(argc == 2);
	BinaryTree_Array<String> bt(atoi(argv[1]));
	...
	while (cmd != 'q')
	{
		...
		case 'p':
		{
			...
			BinaryTree_Array_Iterator_PreOrder<String> iter(bt);
			for ( ; ! iter.done(); iter++)
			{
				cout << iter() << " ";
			}
		...
		case 'o':
		{
			...
			BinaryTree_Array_Iterator_PostOrder<String> iter(bt);
			for ( ; ! iter.done(); iter++)
			{
				cout << iter() << " ";
			}
		...
		case 'n':
		{
			...
			BinaryTree_Array_Iterator_InOrder<String> iter(bt);
			for ( ; ! iter.done(); iter++)
			{
				cout << iter() << " ";
			}
		...
		case 'l':
		{
			...
			BinaryTree_Array_Iterator_LevelOrder<String> iter(bt);
			for ( ; ! iter.done(); iter++)
			{
				cout << iter() << " ";
			}
		...
		{
			cout << endl;
			BinaryTree_Array<String> nbt(bt);
