...
long
stringToLong(const String &a)
{
	long sign = 1;
	long number = 0;
	const char *pb = (const char *)a;
	for ( ; *pb == ' ' || *pb =='\t'; pb++) ;
	switch  (*pb)
	{
	case '-':
	...
	for ( ; *pb != 0 && '0' <= *pb && *pb <= '9'; pb++)
	{
		number = 10*number + (*pb - '0');
	}
	return((sign == -1) ? -number : number);
}

unsigned long
stringToULong(const String &a)
{
	...
	string(s), delimiter(d)
{
	...
	// skip leading delimiters. this does not allow
	// zero-length tokens since two delimiters next to each
	// other are skipped. the following might be in question.
	//
	int is;
