...
// calculate offset into string table for each symbol
static long
calcoffset(int csec, int cstabno)
{
...
readstabs(char *s)
{
...
#endif
	{
		...
		if (pshdrs[sec].sh_link > 0) continue;
		char *psecname = pshstrtbl+pshdrs[sec].sh_name;
		printf("WARNING: invalid sh_link field (%d) for section %d (%s).\n",
			pshdrs[sec].sh_link, sec, psecname);

		// try to find string table section
		char strsecname[BUFSIZ];
		strcpy(strsecname, psecname);
		strcat(strsecname, "str");
		for (int strsec=0; strsec<pehdr->e_shnum; strsec++)
		{
			char *pstrsecname = pshstrtbl+pshdrs[strsec].sh_name;
			if (strcmp(strsecname, pstrsecname) == 0)
			{
				pshdrs[sec].sh_link = strsec;
				printf("FOUND sh_link section (%d, %s) for section (%d, %s).\n",
					strsec, pstrsecname, sec, psecname);
				break;
			}
		}
	}
...
static void
showstab(int &, int &, char *&, char *&, char &, int = 0);

static void
showLineNumbers(const stab *pd, const char *ps, int cstabno, int csec, 
		char *p0, char *pe, char &buf)
{
	Spaces spaces(0);
	for (cstabno++; (p0+cstabno*pshdrs[csec].sh_entsize)<pe; cstabno++)
	{
...
static void
showstab(int &cstabno, int &csec, char *&p0, char *&pe, char &buf, int showlnno)
{
	static int showsec = -1;
	static int showstart = 65565;
	static int showend = -1;
	...
	if (showlnno)
	{
		showLineNumbers(pstab, pstabstr, cstabno, csec, p0, pe, buf);
	}
	else
	{
		// parse the stab string.
...
static void
line_numbers(int &cstabno, int &csec, char *&p0, char *&pe)
{
	char buf;
		...
		for (csec=0; csec<pehdr->e_shnum; csec++)
		{
			     ...
			     pt3=NULL, cstabno++)
			{
				...
				char *pstring = pstr + pstab->n_strx +
					calcoffset(csec, cstabno);
				if (!REequal(pt3, pstring))
					continue;
				showstab(cstabno, csec, p0, pe, buf, 1);
				if (buf == 'q') 
					goto all_done;
				char s[BUFSIZ];
...
static void
review_type(int &cstabno, int &csec, char *&p0, char *&pe)
{
	char buf;
	char save_pt3[BUFSIZ];
	*save_pt3 = '\0';
			...
			for (csec=0; csec<pehdr->e_shnum; csec++)
			{
				     ...
				     cstabno++)
				{
					...
					if (pstab->n_type != search_type)
						continue;
					showstab(cstabno, csec, p0, pe, buf);
					if (buf == 'q') 
						goto all_done2;
					char s[BUFSIZ];
					printf("next stab? [n/y/cr=y] ");
...
static void
review_index(int &cstabno, int &csec, char *&p0, char *&pe)
{
	char buf;
	char save_pt3[BUFSIZ];
	*save_pt3 = '\0';
	...
	else if (strlen(pt2) == strspn(pt2, "0123456789"))
	{
		...
		else 
		{
			...
			// print stab in section
			if ((indexno<0) || 
			   ((p0+indexno*pshdrs[csec].sh_entsize)>=pe))
			{
...
static void
review_other(int &cstabno, int &csec, char *&p0, char *&pe)
{
	char buf;
	char save_pt3[BUFSIZ];
	*save_pt3 = '\0';
...
review(int &cstabno, int &csec, char *&p0, char *&pe)
{
	...
	if (*(pt1+1) == 't')
	{
		review_type(cstabno, csec, p0, pe);
	}
	else if (*(pt1+1) == 'i')
	{
		review_index(cstabno, csec, p0, pe);
	}
	else if (*(pt1+1) == '\0')
	{
		review_other(cstabno, csec, p0, pe);
	}
	else
	{
...
static void
increment(int &cstabno, int &csec, char *&p0, char *&pe, char &buf)
{
	// check section and stab
	if (csec < 0 || csec > pehdr->e_shnum)
	{
...
static void
decrement(int &cstabno, int &csec, char *&p0, char *&pe, char &buf)
{
	// check section and stab
	if (csec < 0 || csec > pehdr->e_shnum)
	{
...
editstabs(char *)
{
	...
	for (int done=0; !done; )
	{
		...
		{
			// printf("unknown cmd.\n");
			increment(cstabno, csec, p0, pe, buf);
		}
		else if (*pt == 'n')
		{
		...
		{
			line_numbers(cstabno, csec, p0, pe);
		}
		else if (*pt == 'r')
		{
		...
		{
			decrement(cstabno, csec, p0, pe, buf);
		}
		else if (*pt == 'q')
		{
			done = 1;
