...
proc usage { } {
	...
	puts "\nusage: [file tail $argv0] \[-\?] \[-x] \[-V] \[-u username] \[-p passwd] \[-U TL1_username] \[-P TL1_passwd] \[labid]";
	puts "where:";
...
proc lcstl1active { ip sp } {
	...
	expect {
	"Telica>" {
		vputs "TL1 prompt received AFTER RTRV-EQPT for $ip ...";
		if (!$imdonemrduck) {
			exp_continue;
		}
	}
	-re "$sp.*:IS,ACT\"" {
		vputs "IS,ACT received AFTER RTRV-EQPT for $ip ...";
		set status "alive - IS,ACT";
		set imdonemrduck 1;
		exp_continue;
	}
	-re "$sp.*:IS,ACT&NBK\"" {
		vputs "IS,ACT,NBK received AFTER RTRV-EQPT for $ip ...";
		set status "alive - IS,ACT,NBK";
		set imdonemrduck 1;
		exp_continue;
	}
	-re "$sp.*:IS,ACT.*SYNC\"" {
		vputs "IS,ACT,SYNC received AFTER RTRV-EQPT for $ip ...";
		set status "alive - IS,ACT,SYNC";
		set imdonemrduck 1;
		exp_continue;
	}
	-re "$sp.*:IS,STBYC" {
		vputs "IS,STBYC received AFTER RTRV-EQPT for $ip ...";
		set status "alive - IS,STBYC";
		set imdonemrduck 1;
		exp_continue;
	}
	-re "$sp.*:IS,STBYC.*SYNC" {
		vputs "IS,STBYC,SYNC received AFTER RTRV-EQPT for $ip ...";
		set status "alive - IS,STBYC,SYNC";
		set imdonemrduck 1;
		exp_continue;
	}
	-re "$sp.*:IS,STBYH" {
		vputs "IS,STBYH received AFTER RTRV-EQPT for $ip ...";
