...
primary:
	...
	{
		/*
		 * strange as it seems, the expression [[],[]], actually
		 * has an interpretation that makes sense:
		 *
		 * [[a,b],[c,d]] = (a+b*i)+(c+d*i)*i)
		 * [[],[]] = a + b*i + c*i + d*i*i
		 * [[],[]] = (a-d) + (b+c)*i
		 * 
		 * since i*i = -1.
		 */
		Complex<double> c2 = number2complex($2);
		Complex<double> c4 = number2complex($4);
		Complex<double> ctotal(0.0, 0.0);
		try {
			ctotal = c2 + Complex<double>(0.0, 1.0)*c4;
		}
		catch (const char *pe)
		{
			yyerror((const char *)"complex [[],[]] failed.");
			YYERROR;
		}
		$$.type = COMPLEX;
		$$.value.cvalue.r = real(ctotal);
		$$.value.cvalue.i = imag(ctotal);
	}
	| LPAREN expr RPAREN
	{
		$$ = $2;
