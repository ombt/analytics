...
proc writedata { stack_name labid current spaces} {
	...
	#
	set bbbdir "$env(LCSBBBSCRIPTS)/${labid}/${current}";
	...
	if {![file isdirectory "${bbbdir}"]} {
		puts "${spaces}==>> creating directory ${bbbdir}.";
		file mkdir ${bbbdir};
	}
	#
	...
	for {set idx $stack(left)} {${idx}<$stack(right)} {incr idx} {
		...
		puts "${spaces}==>> writing ${filename} to BBB file.";
		#
...
#
proc traverse { labid branch current scripts_name stack_name script type number suffix spaces childfound_name { maxchildnumber 9 } } {
		...
		for { } {${childnumber}<=${maxchildnumber}} {incr childnumber} {
			if {[info exists scripts(${childtype}${childnumber})]} {
				...
				foreach childscript $scripts(${childtype}${childnumber}) {
					set status [traverse ${labid} ${branch} ${current} scripts stack $childscript ${childtype} ${childnumber} "${newsuffix}" "${spaces}" newchildfound ];
					if {[isNotOk $status]} {
						return "-1 - traverse: traverse failed:\n${status}";
					}
				}
				# a child may have been found if we return here.
				if {!${newchildfound}} {
					writedata stack $labid $current $oldspaces;
				}
				pop stack dummy;
				myputs "<<< RETURN at may have found a child >>>";
				return "0 - success";
			}
		}
	}
	# no children were found if we return here. this marks
	# the end of the search for this branch. print the data out
	# here.
	writedata stack $labid $current $oldspaces;
	pop stack dummy;
...
proc makebbb { labid branch current { suffix "" } } {
	...
	foreach script $scripts(hardware1) {
		set childfound 0;
		set status [traverse ${labid} ${branch} ${current} scripts stack ${script} hardware 1 "${suffix}" "" childfound ];
		if {[isNotOk $status]} {
