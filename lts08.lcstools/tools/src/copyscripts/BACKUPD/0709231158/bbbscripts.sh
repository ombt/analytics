...
#
proc traverse { labid branch current scripts_name paths_name stack_name script type number suffix spaces childfound_name { maxchildnumber 5 } } {
	upvar ${paths_name} paths;
		...
		for { } {${childnumber}<=${maxchildnumber}} {incr childnumber} {
			...
			if {[info exists scripts(${childtype}${childnumber})]} {
				...
				foreach childscript $scripts(${childtype}${childnumber}) {
					set status [traverse ${labid} ${branch} ${current} scripts paths stack $childscript ${childtype} ${childnumber} "${newsuffix}" "${spaces}" newchildfound ];
					if {[isNotOk $status]} {
...
proc makebbb { labid branch current { suffix "" } } {
	...
	foreach script $scripts(hardware1) {
		...
		set childfound 0;
		set status [traverse ${labid} ${branch} ${current} scripts paths stack ${script} hardware 1 "${suffix}" "" childfound ];
		if {[isNotOk $status]} {
