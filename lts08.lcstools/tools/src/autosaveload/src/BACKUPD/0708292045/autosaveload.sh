...
proc updaterepository { remotemachine username userpasswd branch basedirectory howmany threshold unconditional imagedataname} {
	...
	foreach imagetype [array names imagedata] {
		...
		for {set imgidx 0 } {${imgidx} < ${imglen} && ${imgidx} < ${howmany}} {incr imgidx} {
...
proc usage { } {
	...
	puts "	\[-b base directory] \[-T threshold] \[-n howmany] \[-U]";
	puts "	\[-R remote machine] \[-u username] \[-p passwd] ";
	puts "	\[-t type] branch \[branch2 ...] ]";
	puts "\nwhere:";
	...
	puts "	-T threshold - upper percentage usage limit for repository.";
...
for {set arg 0} {$arg<$argc} {incr arg} {
	...
	{^-T.*} { getoptval $argval threshold arg; }
