...
proc usage { } {
	...
	puts "usage: [file tail $argv0] \[-\?] \[-x] \[-V] \[-K|-X] \[-R] \[-D] \[-T]";
...
proc installnewload { cpu_ip cpu_load ilistname } {
	...
	#
	puts ""
	puts "Removing old /var/TimesTen/sys.ttconnect.init file ...";
	send -i $cpu_id "/bin/rm -f /var/TimesTen/sys.ttconnect.ini 1>/remove.ttconnect.ini.out 2>&1 \r";
	#
	expect {
	-i $cpu_id -re ".*COMMAND_IS_DONE>" {
		# ok
	...
	timeout {
		puts "installnewload: timeout during 'rm -f sys.ttconnect.ini ...'.";
	}
	}
	#
	#
	...
	#
	send -i $cpu_id "./uprev.sh -justinstall\r";
	#
	expect {
	-i $cpu_id -re ".*COMMAND_IS_DONE>" {
		# ok
