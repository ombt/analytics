...
proc auditsp { labid sp ip } {
	...
	ftp_get_ascii_file $ip "$tfile" "$tfile";
	...
	#
	remote_exec $ip_id "find /Telica/sw* -type f -exec ls -l \{\} \\; >$tfile2";
	if {$status != 0} {
		puts "\nremote_exec failed running 'find ...'.";
		return 2;
	}
	ftp_get_ascii_file $ip "$tfile2" "$tfile2";
	#
	# call local audit to compare files.
	#
	system "/home/lcstools/tools/bin/comparefiles $labid $sp $cpu_load $tfile2";
	#
	...
	if {$auditSPA != 0} {
		if {$lcsips(SP_A_IP) != ""} {
			...
			if {$spastatus == "alive"} {
				...
				if {$spastatus == "active"} {
					puts "\nauditing $labid SP_A.";
					auditsp $labid "SP_A" $lcsips(SP_A_IP);
				} else {
	...
	if {$auditSPB != 0} {
		if {$lcsips(SP_B_IP) != ""} {
			...
			if {$spastatus == "alive"} {
				...
				if {$spastatus == "active"} {
					puts "\nauditing $labid SP_B.";
					auditsp $labid "SP_B" $lcsips(SP_B_IP);
				} else {
