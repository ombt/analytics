...
proc clear_remote_fs { machine remotefs } {
	...
	-re ".*COMMAND_IS_DONE>" {
		...
		send "/bin/find $remotefs | /bin/xargs rm -f \r";
	}
	timeout {
		puts "\ntimeout during cd ...";
		timeouthelp;
...
proc killemf { cpu_a_ip cpu_b_ip } {
	...
	if {$cpu_a_ip != "" && $cpu_b_ip != ""} {
		...
		set timeout 10;
		if {$dostopemf} {
			send -i $cpu_a_id "/Telica/swCPU/CurrRel/emf/stopemf\r";
			send -i $cpu_b_id "/Telica/swCPU/CurrRel/emf/stopemf\r";
		} else {
