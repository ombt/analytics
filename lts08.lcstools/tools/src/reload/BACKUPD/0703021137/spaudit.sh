...
proc auditsp { labid sp ip audtype verbose summary results ucond allfiles } {
	...
	#
	log_user 1;
	remote_exec $ip_id "\[ -r /.last_reload ] && echo \"Last Reload: \$(cat /.last_reload)\";";
	log_user 0;
	#
	if {$loadonly} {
	...
	#
		...
		system "$env(LCSTOOLSBIN)/comparefiles $labid $sp $cpu_load $tfile2 $audtype $verbose $summary $results $ucond $allfiles";
	} ignore;
