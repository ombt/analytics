...
proc getimages2load { labid reloadcpuload bname lname ilistname } {
	...
	#
	if {[dbselect iobuf images "branch req ^$branch\$ and cpuload req ^$cpuload\$ and type rne ^bts\$ and type rne ^tca\$ and type rne debug_cpu" "type name" ] != 0} {
...
proc loadkludge { cpu_ip telnet_spawn_id { timeout 20 } } {
	#
	ftp_put_binary_file $cpu_ip "/lcsl101/text/Main/cpu/partition_create_extended" "/Telica/swCPU/CurrRel/system/scripts/partition_create_extended";
	#
	send -i $telnet_spawn_id "chmod 777 /Telica/swCPU/CurrRel/system/scripts/partition_create_extended\r";
	#
	expect {
