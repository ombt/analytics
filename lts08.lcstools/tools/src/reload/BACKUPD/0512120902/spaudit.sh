...
#
proc auditsp { labid sp ip audtype verbose summary results } {
	#
	global env;
	global loadonly;
	#
	...
	#
		...
		# system "/home/lcstools/tools/bin/comparefiles $labid $sp $cpu_load $tfile2 $audtype $verbose $summary $results";
		system "$env(LCSTOOLSBIN)/comparefiles $labid $sp $cpu_load $tfile2 $audtype $verbose $summary $results";
	} ignore;
	#
	...
	if {$auditSPA != 0} {
		if {$lcsips(SP_A_IP) != ""} {
			...
			if {$spastatus == "alive"} {
				...
				if {$spastatus == "active"} {
					puts "\nauditing $labid SP_A.";
					auditsp $labid "SP_A" $lcsips(SP_A_IP) $audtype $verbose $summary $results;
				} else {
	...
	if {$auditSPB != 0} {
		if {$lcsips(SP_B_IP) != ""} {
			...
			if {$spastatus == "alive"} {
				...
				if {$spastatus == "active"} {
					puts "\nauditing $labid SP_B.";
					auditsp $labid "SP_B" $lcsips(SP_B_IP) $audtype $verbose $summary $results;
				} else {
...
if {$arg >= $argc} {
	...
	for { } {$arg<$argc} {incr arg} {
		...
		logmsg lcsspaudit "START-AUDIT: labid=$labid, type=$audtype, doSPA=$auditSPA, doSPB=$auditSPB";
		faudit $labid $audtype;
		logmsg lcsspaudit "END-AUDIT: labid=$labid, type=$audtype, doSPA=$auditSPA, doSPB=$auditSPB";
	}
}
#
# done
