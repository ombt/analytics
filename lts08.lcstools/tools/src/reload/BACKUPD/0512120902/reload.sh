...
proc usage { } {
	...
	puts "usage: [file tail $argv0] \[-\?] \[-x] \[-V] \[-K|-X|-H] \[-R] \[-D] \[-T]";
...
proc removeoldload { cpu_ip ilistname } {
	...
	#
		...
		# remove old TimesTen log files from when the 
		# the data was first installed.
		set timeout 300;
		puts ""
		puts "Removing old TimesTen log files /Telica/dbCurrent/SoftSwitch*/* ...";
		send -i $cpu_id "rm -rf /Telica/dbCurrent/SoftSwitch*/* 1>/dev/null 1>&2\r";
		#
		expect {
		-i $cpu_id -re ".*COMMAND_IS_DONE>" {
			# ok
		...
		timeout {
			puts "removeoldload: timeout during rm of /Telica/dbCurrent/SoftSwitch*.";
		}
		}
		#
		puts ""
		puts "Removing old directories /Telica/dbCurrent/Iom_* ...";
		send -i $cpu_id "rm -rf /Telica/dbCurrent/Iom_* 1>/dev/null 1>&2\r";
		#
		expect {
		-i $cpu_id -re ".*COMMAND_IS_DONE>" {
			# ok
		...
		timeout {
			puts "removeoldload: timeout during rm of /Telica/dbCurrent/Iom_*.";
		}
		}
		#
		puts ""
		puts "Removing old backup directories /Telica/dbCurrent/backup* ...";
		send -i $cpu_id "rm -rf /Telica/dbCurrent/backup* 1>/dev/null 1>&2\r";
		#
		expect {
		-i $cpu_id -re ".*COMMAND_IS_DONE>" {
			# ok
		...
		timeout {
			puts "removeoldload: timeout during rm of /Telica/dbCurrent/backup*.";
		}
		}
		#
		puts ""
		puts "Removing old database init file /.odbc.ini ...";
		send -i $cpu_id "rm -f /.odbc.ini 1>/dev/null 1>&2\r";
		#
		expect {
		-i $cpu_id -re ".*COMMAND_IS_DONE>" {
			# ok
		...
		timeout {
			puts "removeoldload: timeout during rm of /.odbc.ini.";
		}
		}
		# remove any last vestiges of TimesTen.
		set timeout 300;
		puts ""
		puts "Removing any TimesTen left-overs under /opt ...";
		send -i $cpu_id "rm -rf /opt/TimesTen* 1>/dev/null 1>&2\r";
		#
		expect {
		-i $cpu_id -re ".*COMMAND_IS_DONE>" {
			# ok
		...
		timeout {
			puts "removeoldload: timeout during rm of /opt/TimesTen*.";
		}
		}
		puts ""
		puts "Clearing /etc/resolv.conf (disabling DNS) ...";
		send -i $cpu_id "echo '# over-written by reload' >/etc/resolv.conf\r";
		#
		expect {
		-i $cpu_id -re ".*COMMAND_IS_DONE>" {
			# ok
		...
		timeout {
			puts "removeoldload: timeout during clearing of /etc/resolv.conf.";
		}
		}
	} ignore;
	#
