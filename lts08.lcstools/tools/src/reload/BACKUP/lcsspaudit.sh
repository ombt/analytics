...
#
proc auditsp { labid sp ip audtype } {
	#
	...
	#
		...
		system "/home/lcstools/tools/bin/comparefiles $labid $sp $cpu_load $tfile2 $audtype";
	} ignore;
	#
	...
	if {$auditSPA != 0} {
		if {$lcsips(SP_A_IP) != ""} {
			...
			if {$spastatus == "alive"} {
				...
				if {$spastatus == "active"} {
					puts "\nauditing $labid SP_A.";
					auditsp $labid "SP_A" $lcsips(SP_A_IP) $audtype;
				} else {
	...
	if {$auditSPB != 0} {
		if {$lcsips(SP_B_IP) != ""} {
			...
			if {$spastatus == "alive"} {
				...
				if {$spastatus == "active"} {
					puts "\nauditing $labid SP_B.";
					auditsp $labid "SP_B" $lcsips(SP_B_IP) $audtype;
				} else {
