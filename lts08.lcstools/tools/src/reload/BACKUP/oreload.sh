...
proc getimages2load { labid reloadcpuload bname lname ilistname } {
	...
	if {$unconditional} {
		...
		if {![info exists chiomsbuf] || [llength chiomsbuf] == 0} {
			set iomdataexists 0;
			puts "Chassis-To-IOM-Type Data for $labid: none available, reload everything.";
			logmsg reload "Chassis-To-IOM-Type Data for $labid: none available, reload everything.";
		} else {
			# always add cpu
			lappend chiomsbuf cpu;
			set iomdataexists 1;
			puts "Chassis-To-IOM-Type Data for $labid: $chiomsbuf";
			logmsg reload "Chassis-To-IOM-Type Data for $labid: $chiomsbuf";
		}
	} else {
...
proc loadprivprods { cpu_ip } {
	...
	foreach fromfile $fromfiles {
		...
		puts "To file   : $tofile";
		#
		puts "\nRemoving file $tofile on $cpu_ip ..."
		remove_remote_file $cpu_ip $tofile 0;
		#
...
proc installnewload { cpu_ip cpu_load ilistname } {
	...
	if {[info exists tarfiledata(cpu)]} {
		...
		puts "Linking $priv_cpu_load to CurrRel ...";
		send -i $cpu_id "/bin/ln -s /Telica/swCPU/${priv_cpu_load} /Telica/swCPU/CurrRel\r";
	} else {
...
proc preload { labid cpu_ip } {
	global env;
	#
	ftp_put_binary_file $cpu_ip $env(LCSTOOLSLYNXBIN)/rm /bin/rm;
	telnet_to $cpu_ip cpu_id;
	send -i $cpu_id "/bin/chmod 755 /bin/rm\r";
	expect {
	-i $cpu_id -re ".*COMMAND_IS_DONE>" {
		# ok
	}
	timeout {
		puts "\nERROR: removeoldload: timeout waiting for chmod 755 /bin/rm.";
		set spawn_id $cpu_id;
		catch { close; } ignore;
		catch { exec kill -9 [exp_pid]; } ignore;
		catch { wait; };
		logmsg reload "END: preload: timeout waiting for chmod 755 /bin/rm.";
		exit 2;
	}
	eof {
		puts "\nERROR: removeoldload: eof waiting for chmod 755 /bin/rm.";
		set spawn_id $cpu_id;
		catch { close; } ignore;
		catch { exec kill -9 [exp_pid]; } ignore;
		catch { wait; };
		logmsg reload "END: preload: eof waiting for chmod 755 /bin/rm.";
		exit 2;
	}
	}
	#
