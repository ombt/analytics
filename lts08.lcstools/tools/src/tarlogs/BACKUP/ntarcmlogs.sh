...
proc usage { } {
	...
	puts "	\[-F ftp_path] \[cm_slot#] \[cm_slot# ...] ";
	puts "";
	puts "where:";
...
proc getcmlogfile { labid activesp activeip cm ftppath } {
	...
	for {set cpu 1} {$cpu<=4} {incr cpu} {
		...
		remote_exec $ip_id "rsh $cm_cpu_ip \"tar czf - /procLogCore/log /procLogCore/oldLog*\" >/home/${labid}_cm${cm}_cpu${cpu}_log.tar.gz " 600;
	}
	remote_exec $ip_id "tar cf /home/${labid}_cm${cm}_allcpu_log.tar /home/${labid}_cm${cm}_cpu*_log.tar.gz " 600;
