...
#
proc getsplogfile { labid activesp activeip ftppath } {
	global sp_cpu_ips;
	global ftp_setuid;
	#
	puts "";
	...
	remote_exec $ip_id "tar czvf /home/${labid}_${activesp}_core.tar.gz /home1/Telica/swCPU/core" 600;
	#
	close_telnet $ip_id;
	#
	if {$ftppath != ""} {
		set ftp_setuid 0;
		puts "";
		puts "FTP'ing remote file: /home/${labid}_${activesp}_core.tar.gz";
		puts "To local file: ${ftppath}/${labid}_${activesp}_core.tar.gz";
		ftp_get_binary_file $activeip "/home/${labid}_${activesp}_core.tar.gz" "${ftppath}/${labid}_${activesp}_core.tar.gz";
		ftp_chmod "${ftppath}/${labid}_${activesp}_core.tar.gz" 0777;
	}
}
#
#########################################################################
...
if {$arg >= $argc} {
	...
	if {$sp == "A" || $sp == "both"} {
		...
		getsplogfile $labid "SP-A" $cpu_a_ip $ftppath;
	} 
	if {$sp == "B" || $sp == "both"} {
		puts "";
		puts "Retrieving SP-B log files for $labid:";
		puts "All log files are under SP-B /home.";
		getsplogfile $labid "SP-B" $cpu_b_ip $ftppath;
	}
} else {
