...
proc usage { } {
	...
	puts "\nusage: [file tail $argv0] \[-\?] \[-x] \[-B] \[-R] \[-U] \[-V] \[-S trace level]";
...
#
proc sendemail { branch operation status msg } {
	global env;
	set cmd "/usr/bin/ksh $env(LCSTOOLSBIN)/lcssendemail $branch $operation $status \"$msg\" ";
	# puts "$cmd";
