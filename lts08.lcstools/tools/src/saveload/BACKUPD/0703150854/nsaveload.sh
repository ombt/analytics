...
proc getimages { branch imagesname { debugflag "" } } {
	...
	ftp_put_binary_file $machine "${toolsbin}/nload2images" "/tmp/nload2images${mypid}";
	# run it remotely
	set outfile "/tmp/nload2images.out.${mypid}";
	#
	telnet_to $machine remote_ip;
	remote_exec $remote_ip "/bin/chmod 777 /tmp/nload2images${mypid}";
	#
	...
	#
	remote_exec $remote_ip "/tmp/nload2images${mypid} ${debugflag} ${branch} $btype all  ${btsbranch} 1>${outfile} 2>&1";
	# get results
	ftp_get_binary_file $machine "${outfile}" "${outfile}";
	# delete old files
	remote_exec $remote_ip "/bin/rm -f /tmp/nload2images${mypid} ${outfile} 1>/dev/null 2>&1";
