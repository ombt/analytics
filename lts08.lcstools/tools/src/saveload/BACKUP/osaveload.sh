...
proc usage { } {
	...
	puts "	\[-d] \[-G] \[-b branch_to_save] \[-u username] \[-p passwd] \[-t status] \[labid \[labid ...] ]";
	puts "where:";
	...
	puts "	-r remote_machine!full_path - expand CPU DEBUG files on remote machine at the given path.";
	puts "";
...
proc getimages { branch imagesname { debugflag "" } } {
	...
	ftp_put_binary_file $machine "${toolsbin}/oload2images" "/tmp/oload2images${mypid}";
	# run it remotely
	set outfile "/tmp/oload2images.out.${mypid}";
	#
	telnet_to $machine remote_ip;
	remote_exec $remote_ip "/bin/chmod 777 /tmp/oload2images${mypid}";
	#
	...
	#
	remote_exec $remote_ip "/tmp/oload2images${mypid} ${debugflag} ${branch} $btype all  1>${outfile} 2>&1";
	# get results
	ftp_get_binary_file $machine "${outfile}" "${outfile}";
	# delete old files
	remote_exec $remote_ip "/bin/rm -f /tmp/oload2images${mypid} ${outfile} 1>/dev/null 2>&1";
...
proc shipdebugfile { imagetype tofile } {
	...
	remote_exec $remote_id "/bin/rm -f ${rdir}/${tarfile} 2>/dev/null 1>&1";
	remote_exec $remote_id "/bin/rm -rf ${rdir}/Telica/swCPU/${global_cpuload}_* 2>/dev/null 1>&1";
	close_telnet $remote_id;
	#
	# ship the new debug tar file over.
	#
	ftp_put_binary_file "${rmach}" "${tofile}" "${rdir}/${tarfile}";
	#
	# expand the remote tar file.
	#
	telnet_to ${rmach} remote_id;
	remote_exec $remote_id "cd ${rdir}";
	remote_exec $remote_id "/bin/tar xzvf ${rdir}/${tarfile}" 1500;
