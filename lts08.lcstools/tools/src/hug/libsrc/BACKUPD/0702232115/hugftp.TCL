...
#
proc hug_ftp_open { cfgdata streamid ip login passwd {vflag 0} {tmout 30} } {
	#
	upvar $cfgdata cfgd;
	#
	global timeout;
	...
	timeout {
		return "-1 - hug_ftp_open: time out during ftp ($ip,$login).";
	}
	eof {
		return "-1 - hug_ftp_open: eof during ftp ($ip,$login).";
	}
	}
	#
	expect {
...
proc hug_ftp_get { cfgdata streamid frompath topath {asciiflag 0} {binaryflag 1} {ignorefilenotfound 0} {vflag 0} {tmout 30} } {
	...
	timeout {
		return "-1 - hug_ftp_get: time out during binary or ascii operation ($ip,$login).";
	}
	eof {
		return "-1 - hug_ftp_get: eof during binary or ascii operation ($ip,$login).";
	}
	}
	#
	# time out value
...
proc hug_ftp_put { cfgdata streamid frompath topath {asciiflag 0} {binaryflag 1} {ignorefilenotfound 0} {vflag 0} {tmout 30} } {
	...
	timeout {
		return "-1 - hug_ftp_put: time out during binary or ascii operation ($ip,$login).";
	}
	eof {
		return "-1 - hug_ftp_put: eof during binary or ascii operation ($ip,$login).";
	}
	}
	#
	# time out value
