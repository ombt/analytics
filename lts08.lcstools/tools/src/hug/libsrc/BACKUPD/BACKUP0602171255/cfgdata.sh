...
#
proc readAndParseCfgDataFile { cfgdata cmddata cfgdidx block blocknames cfgfile } {
	upvar $cfgdata cfgd;
	upvar $cmddata cmdd;
	upvar $cfgdidx cidx;
	upvar $block blk;
	...
	for {set lnno 1} {[gets $fd inbuf] > 0} {incr lnno} {
		...
		# parse command line.
		#
		if {[regexp -- {^([a-zA-Z_0-9]+)[ \t]+([a-zA-Z_0-9]+)[ \t]+\{[ \t]*$} $cmd mstr blockname blocktype ] == 1} {
			...
			if {$blk == 1} {
				...
				return "-1 - readAndParseCfgDataFile: nexted blocks not allowed for exec block at lnno $lnno in $cfgfile";
			}
...
#
proc readCfgDataFile { cfgdata cmddata cfgfile } {
	upvar $cfgdata cfgd;
	upvar $cmddata cmdd;
	#
	set cfgdidx "";
	...
	#
	set status [readAndParseCfgDataFile cfgd cmdd cfgdidx block blocknames $cfgfile]
	if {[isNotOk $status]} {
