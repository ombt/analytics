...
#
proc hug_telnet_open { cfgdata streamid vflag tmout ip login passwd } {
	#
	upvar $cfgdata cfgd;
	#
	global spawn_id;
	...
	timeout {
		...
		return "-1 - hug_telnet_open: time out during telnet ($ip,$login).";
	}
	eof {
		# catch { close $cfgd(telnet,$streamid,spawn_id); } ignore;
		hug_telnet_killstream cfgd $streamid;
		return "-1 - hug_telnet_open: eof during telnet ($ip,$login).";
	}
	}
	#
	expect {
...
proc hug_telnet_exec { cfgdata streamid iflag vflag tmout xcmd { rflag 0 } } {
	...
	if {$rflag == 0 } {
		...
		return "-1 - failure because of reverse flag";
	}
}
#
proc hug_telnet_close { cfgdata streamid vflag tmout } {
	...
	#
	if {![info exists cfgd(telnet,$streamid,spawn_id)]} {
		# return "-1 - hug_telnet_close: stream id $streamid does not exist.";
		return "0 - success";
	}
	if {$tmout != 0} {
		set timeout $tmout;
