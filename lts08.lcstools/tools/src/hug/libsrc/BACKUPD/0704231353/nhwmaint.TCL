...
proc hw_maint_rmv_all_ioms { labid spaip spbip username passwd { maxiom 17 } { polltime 30 } { maxtime 1200 } { maxresets 5 } } {
	...
	for {set done 0} { ! $done && $maxiters>0 } {sleep $polltime} {
		...
		if {[isNotOk $status]} {
			hw_maint_close $spid;
			puts "\nERROR: hw_maint_rmv_all_ioms: hw_maint_rtrv_eqpt failed:\n${status}";
			puts "\nRe-opening TL1 session for ($labid,$spaip,$spbip).";
			sleep 60;
			set status [hw_maint_tl1_open spid $spaip $spbip $username $passwd]
			if {[isNotOk $status]} {
				return "-1 - hw_maint_rmv_all_ioms: hw_maint_tl1_open failed (re-open):\n${status}";
			}
			set spawn_id $spid;
			incr maxiters -1;
			continue;
...
proc hw_maint_rst_all_ioms { labid spaip spbip username passwd { maxiom 17 } { polltime 30 } { maxtime 1200 } { maxresets 5 } } {
	...
	for {set done 0} { ! $done && $maxiters>0 } {sleep $polltime} {
		...
		set status [hw_maint_rtrv_eqpt spid data];
		if {[isNotOk $status]} {
			puts  "\nERROR: hw_maint_rst_all_ioms: hw_maint_rtrv_eqpt failed:\n${status}";
			puts "\nRe-opening TL1 session for ($labid,$spaip,$spbip).";
			sleep 60;
			set status [hw_maint_tl1_open spid $spaip $spbip $username $passwd]
			if {[isNotOk $status]} {
				return "-1 - hw_maint_rst_all_ioms: hw_maint_tl1_open failed (re-open):\n${status}";
			}
			set spawn_id $spid;
			incr maxiters -1;
			continue;
...
#
proc hw_maint_update_ioms { labid spaip spbip { tl1username "telica" } { tl1passwd "telica" } { lynxusername "root" } { lynxpasswd "plexus9000" } { iomstates "oosaumaflt,oosauflt" } { iomlist "" } { allflag 1 } { maxiom 17 } } {
	#
