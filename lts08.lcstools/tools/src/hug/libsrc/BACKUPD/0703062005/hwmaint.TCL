...
proc hw_maint_rmvallioms { spid { maxiom 17 } { polltime 30 } { maxtime 1200 } { maxresets 5 } } {
			...
			if {$data(iom,$iom,rmvstate) == "NOTSTARTED"} {
				...
				-re "DENY.*Not in Valid State.*;\r\nTelica>" {
					...
					if {$data(iom,$iom,resetcounter) > $maxresets} {
						...
						log_user 0;
						hw_maint_close $spid;
						return "-1 - hw_maint_rmvallioms: Maximum attempts of RMV-EQPT::IOM-$iom exceeded.";
					}
					# increment secrmvsinprog so we retry
					# again. see below.
				}
				...
				eof {
					...
					log_user 0;
					hw_maint_close $spid;
					return "-1 - hw_maint_rmvallioms: RMV-EQPT::IOM-$iom EOF";
				}
				}
				incr secrmvsinprog;
				log_user 0;
	...
	if {!$done} {
		...
		puts "ERROR: time out. unable to remove all IOMs";
		return "-1 - hw_maint_rmvallioms: time out. unable to remove all IOMs";
	} else {
		return "0 - success";
	}
}
...
# restore all IOMs
#
proc hw_maint_rstallioms { spid { maxiom 17 } { polltime 30 } { maxtime 1200 } { maxresets 5 } } {
	upvar $spid spawn_id;
	# init some data
		...
		for {set iom 1} {$iom<=$maxiom} {incr iom} {
			...
			if {$data(iom,$iom,rststate) == "NOTSTARTED"} {
				...
				eof {
					...
					log_user 0;
					hw_maint_close $spid;
					return "-1 - hw_maint_rstallioms: RST-EQPT::IOM-$iom EOF";
				}
				}
				log_user 0;
				set data(iom,$iom,rststate) "INPROG";
	...
	if {!$done} {
		...
		log_user 0;
		puts "ERROR: time out. unable to restore all IOMs";
		return "-1 - hw_maint_rstallioms: time out. unable to restore all IOMs";
	} else {
		return "0 - success";
	}
}
...
proc hw_maint_get_cpuload { labid spaip spbip username passwd argdata argbranch argcpuload arglastbranch arglastreload { timeout 30 } } {
	...
	if {[string length $lastreload] > 0} {
		...
		if {![info exists obuf] || [llength obuf] == 0} {
			return "-1 - hw_maint_get_cpuload: No BRANCH found for CPULOAD $lastreload.";
		}
...
proc hw_maint_iom_rsh_open { spid ip { timeout 30 } } {
	...
	timeout {
		...
		log_user 0;
		return "-1 - hw_maint_iom_rsh_open: timeout from rsh to ${ip}.";
	}
	eof {
		log_user 0;
...
#
proc hw_maint_update_ioms { labid spaip spbip { tl1username "telica" } { tl1passwd "telica" } { lynxusername "root" } { lynxpasswd "plexus9000" } { iomstates "oosaumaflt" } { iomlist "" } { maxiom 17 } } {
	#
	...
	if {[isNotOk $status]} {
		...
		return "-1 - hw_maint_update_ioms: hw_maint_rtrv_eqpt failed:\n${status}";
	}
	hw_maint_close $tl1spid;
	...
	for {set idx 1} {$idx<=$iomcnt} {incr idx} {
		...
		#
		if {[info exists cleis($realbranch,$clei)]} {
			set type $cleis($realbranch,$clei);
		} elseif {[info exists cleis(default,$clei)]} {
			set type $cleis(default,$clei);
		} else {
			return "-1 - hw_maint_update_ioms: Unknown CLEI value $clei for IOM $iom";
		}
			...
			foreach fp $flashparts(minimal,$iomtype) {
				...
				if {[llength $linkpath] > 1} {
					...
					if {$type != "vs3"} {
						...
						hw_maint_close $spawn_id;
						return "-1 - hw_maint_update_ioms: multiple files in one tar file, and not 'vs3'; it's $type.";
					}
...
proc hw_maint_cpymemallioms { spid { maxcpymems 3 } { recoverioms 0 } { maxiom 17 } { polltime 30 } { maxtime 1800 } { maxresets 5 } } {
	...
	if {[string length "${notoosioms}"] > 0} {
		return "-1 - hw_maint_cpymemallioms: IOMs ${notoosioms} are not OOS,MA.";
	} elseif {$iomcnt <= 0} {
		return "-1 - hw_maint_cpymemallioms: NO IOMs found to CPY-MEM.";
	} else {
		puts -nonewline "\n$iomcnt IOMs to CPY-MEM are: ";
		for {set idx 1} {$idx<=$iomcnt} {incr idx} {
			puts -nonewline "$ioms($idx) ";
		...
		for {set idx $minidx} {$idx<=$maxidx} {incr idx} {
			...
			if {$data(iom,$iom,cpymemstate) == "NOTSTARTED"} {
				...
				eof {
					puts "\nERROR: CPY-MEM::IOM-$iom EOF";
					log_user 0;
					return "-1 - hw_maint_cpymemallioms: CPY-MEM::IOM-$iom EOF";
				}
				}
				log_user 0;
				set data(iom,$iom,cpymemstate) "INPROG";
	...
	if {!$done} {
		...
		log_user 0;
		return "-1 - hw_maint_cpymemallioms: time out. unable to CPY-MEM all IOMs";
	} else {
		return "0 - success";
	}
}
