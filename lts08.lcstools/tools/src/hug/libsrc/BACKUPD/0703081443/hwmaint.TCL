...
#
proc hw_maint_update_ioms { labid spaip spbip { tl1username "telica" } { tl1passwd "telica" } { lynxusername "root" } { lynxpasswd "plexus9000" } { iomstates "oosaumaflt" } { iomlist "" } { allflag 1 } { maxiom 17 } } {
	#
	...
	for {set idx 1} {$idx<=$iomcnt} {incr idx} {
		...
		for {set cpu $mincpu} {$cpu<=$maxcpu} {incr cpu} {
			...
			foreach fp $fps {
				...
				#
				set updcmd "/application/update_flash_partition --v ${activespip} ${linkpath} ${realftype}";
				puts "\t\tFTYPE: $ftype ($realftype)";
				puts "\t\tFPATH: $linkpath";
				puts "\t\tUPD CMD: $updcmd";
