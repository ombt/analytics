...
proc exectl1 { cfgdata section cmd xcmd } {
	...
	#
	set oper [lindex $argv 0];
	if {[lsearch -exact {open open2 exec file close rmvallioms rstallioms cpymems} $oper] == -1} {
...
proc generateGlobals { cfgdata cmddata spdata iomdata} {
	...
	set cmdd(globals,[incr cmd]) "textdir $cfgd(globals,load,textdir)";
	...
	foreach image $cfgd(globals,load,images,types) {
		set cmdd(globals,[incr cmd]) "$image $cfgd(globals,load,images,$image)";
...
proc generatePreHugIomCmds { cfgdata cmddata spdata iomdata command } {
	...
	foreach iomtype $iomd(types) {
		...
		set cmdd(prehug,[incr cmd]) "tl1 exec -t900 -v prehugtl1 exec-ftp-get::com:::<ftpmachine>,<ftplogin>,<ftppasswd>,<textdir>/$iomtype/<$iomtype>,<<activesp>.newcurrrel>";
...
proc generatePreHugCmds { cfgdata cmddata spdata iomdata { huglite 0 } } {
	...
	#
	set cmdd(prehug,[incr cmd]) "tl1 open -t30 prehugtl1 <<activesp>.ip> <tl1login> <tl1passwd>";
	set cmdd(prehug,[incr cmd]) "tl1 exec -t900 -v prehugtl1 exec-ftp-get::com:::<ftpmachine>,<ftplogin>,<ftppasswd>,<textdir>/cpu/<cpu>,<<activesp>.newcurrrel>";
	set cmdd(prehug,[incr cmd]) "sleep <interunixcmddelay>";
...
#
proc generateNewCmLoadHugCmds { cmddata cmdno standbycm activecm } {
	upvar $cmddata cmdd;
	upvar $cmdno cmd;
	#
...
#
proc generateNewCpuLoadCmHugCmds { cmddata cmdno standbycm activecm } {
	upvar $cmddata cmdd;
	upvar $cmdno cmd;
	#
	set cmdd(cmhug,[incr cmd]) \
...
proc generateCmHugCmds { cfgdata cmddata spdata iomdata} {
	...
	foreach lowcm {14 1 3 5 7 10 12} {
		...
		#
		if {$newcmload} {
			generateNewCmLoadHugCmds cmdd cmd $standbycm $activecm;
		} else {
			generateNewCpuLoadCmHugCmds cmdd cmd $standbycm $activecm;
		}
	}
	#
	set cmdd(cmhug,[incr cmd]) "}";
...
proc generateHugCmds { cfgdata cmddata { huglite 0 } } {
	...
	for {set iom 1} {[info exists cfgd(iom,$iom,iomstate)]} {incr iom} {
		...
		if {[info exists cleis($branch,$clei)]} {
			...
			set iomdata($iom,type) $cleis(default,$clei);
