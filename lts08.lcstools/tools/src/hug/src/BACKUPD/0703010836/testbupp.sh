...
#
usage="\n
usage: ${CMD} [-?] [-x] [-E] [-X] [-I] [-P] [-G] \n
\t[-b branch] [-c cpuload] [-m email1[,email2,...]] \n
\t[-i [duplex | simplex [-A|-B] ] [-o outfile] [-U user TL1 scripts path]\n
\t[-T script1,script2,...] [-t] [-d] [-e] [-f] [-L] [labid] \n
\n
where: \n
\t-x - debug \n
\t-? - usage message \n
...
\t-u uesrmapfile - file mapping user products to chassis products \n
\n
...
takes precedence. \n
\n 
If the -u option is used, then the user map file must have this format: \n
\n
path_to_user_product1 path_chassis_product1 \n
path_to_user_product2 path_chassis_product2 \n
... \n
Immediately after the tar files are expanded, then all the files in the user\n
map file are installed via ftp.
"
#
branch=
cpuload=
...
usermapfile=
#
set -- `getopt ?Xfetxb:o:c:i:T:U:u:LGPIEABm:d ${*}`
if [[ ${?} -ne 0 ]]
then
	echo $usage >&2
	exit 2
...
do
	...
	-u)
		...
		if [[ ! -r "${usermapfile}" ]]
		then
			echo "User map file ${usermapfile} is not readable."
			exit 2
		fi
		shift 2
		;;
...
then
	...
	then
		echo
		echo "Log file for ${labid} is not readable."
		#
		exit 2
	fi
	#
...
then
	...
	select branch in QUIT $(uprintf -q -f"%s\n" branch in ${LCSTOOLSDATA}/nlabloads where labid req ^${labid}$ ${statusclause} | sort -u )
	do
		case "${branch}" in
		QUIT)
			echo "Quitting ${CMD}."
...
then
	...
	select cpuload in QUIT $(uprintf -q -f"%s\n" cpuload in ${LCSTOOLSDATA}/nlabloads where labid req ^${labid}$ and branch req ^${branch}$ ${statusclause} | sort -u )
	do
		case "${cpuload}" in
		QUIT)
			echo "Quitting ${CMD}."
...
writeglobals () {
	...
	then
		echo
		echo "SP-A IP is null." >&2
		exit 2
	elif [[ -z "$spb_ip" ]]
	then
		echo
		echo "SP-B IP is null." >&2
		exit 2
	fi
	#
	cat >>$outfile <<EOF
...
EOF
	...
	then
		echo
		echo "File system for branch $branch is unknown." >&2
		exit 2
	fi
	#
...
writeftpload () {
	...
	#
	uprintf -q -f"%s\n" type in ${LCSTOOLSDATA}/chassis2ioms where branch req ^${branch}$ and labid req ^${labid}$ | 
	sort -u >${iomtypes}
	#
	if [[ -s "${iomtypes}" ]]
	then
		...
		echo
		echo "No data found in chassis2ioms. Using images relation."
		uprintf -q -f"%s\n" type in ${LCSTOOLSDATA}/images where branch req ^${branch}$ and cpuload req ^${cpuload}$ | 
		sort -u >${iomtypes}
	fi
	#
	echo
...
EOF
	...
	then
		...
		do
			...
			then
				echo
				echo "User product is not readable: ${userprod}"
				exit 2
			fi
			echo "	telnet exec -t120 -i -v telnetspa /bin/rm -f ${ofcprod}" >>$outfile
			echo "	telnet exec -t120 -i -v telnetspb /bin/rm -f ${ofcprod}" >>$outfile
...
writeprovisioning () {
	...
	then
		...
		do
			...
			then
				echo
				echo "Script ${tl1dir}/${script} is not readable."
				exit 2
			fi
		done
	else
		#
		# let user choose which scripts to execute for now. later,
		# choose for them.
		#
		cd ${tl1dir}
		#
		...
		do
			if [[ -f "${script}" ]]
			then
				availablescripts="$availablescripts $script"
			fi
		done
		#
		if [[ -z "${availablescripts}" ]]
		then
			echo
		...
		oper=add
		scripts2exec=
		PS3="Choose scripts and order to execute (oper=$oper):"
		select script in QUIT DONE_CHOOSING SKIP_PROVISIONING ADD DEL ${availablescripts}
		do
			case "${script}" in
			QUIT)
				echo
			...
			*)
				...
				add)
					...
					echo "Current scripts to run: ${scripts2exec}"
					echo
					;;
				del)
					echo
					...
					#
					newscripts2exec=
					for currscript in ${scripts2exec}
					do
						if [[ "${currscript}" == "${script}" ]]
						then
							echo "removing script: ${currscript}"
						else
							echo "keeping script: ${currscript}"
							newscripts2exec="${newscripts2exec} ${currscript}"
						fi
					done
					scripts2exec="${newscripts2exec}"
					echo "Current scripts to run: ${scripts2exec}"
					echo
					;;
				esac
...
EOF
		...
		*)
		...
		then
			...
			echo "	tl1 file -v tl1sp ${tl1dir}/${script}" >>${duplexscript}
			echo "	tl1 file -v tl1sp ${tl1dir}/${script}" >>${simplexspascript}
			echo "	tl1 file -v tl1sp ${tl1dir}/${script}" >>${simplexspbscript}
		else
			...
			#
			# strip out init-sys commands and run temporary
			# files.
			#
			grep -v init-sys: ${script} >/lcsl100/tmp/${script}.$$
			echo "	tl1 file -v tl1sp /lcsl100/tmp/${script}.$$" >>${simplexspascript}
			echo "	tl1 file -v tl1sp /lcsl100/tmp/${script}.$$" >>${simplexspbscript}
			#
			cat >>${simplexspascript} <<EOF
	tl1 close tl1sp
	#
...
EOF
		...
		*hardware*)
			...
			then
				...
				# first batch of IOMS
				# rmv the ioms
				for iomlist in "${iomlist1}" "${iomlist2}"
				do
					if [[ -z "${iomlist}" ]]
					then
						# skip null length strings
						continue
					fi
					#
					for iom in ${iomlist}
					do
						echo "	tl1 exec -v tl1sp rmv-eqpt::iom-${iom}" >>${duplexscript}
						echo "	tl1 exec -v tl1sp rmv-eqpt::iom-${iom}" >>${simplexspascript}
						echo "	tl1 exec -v tl1sp rmv-eqpt::iom-${iom}" >>${simplexspbscript}
					done
					# wait for the IOM to OOS. wait sequentially.
					# remember, the slowest one will dominate.
					for iom in ${iomlist}
					do
						echo "	waitforiomstate -t1200 tl1sp ${iom} oosma,oos" >>${duplexscript}
						echo "	waitforiomstate -t1200 tl1sp ${iom} oosma,oos" >>${simplexspascript}
						echo "	waitforiomstate -t1200 tl1sp ${iom} oosma,oos" >>${simplexspbscript}
					done
					# now cpy-mem the IOMs.
					for iom in ${iomlist}
					do
						echo "	tl1 exec -v tl1sp cpy-mem::iom-${iom}" >>${duplexscript}
						echo "	tl1 exec -v tl1sp cpy-mem::iom-${iom}" >>${simplexspascript}
						echo "	tl1 exec -v tl1sp cpy-mem::iom-${iom}" >>${simplexspbscript}
					done
					# wait for cpy-mem to finish.
					for iom in ${iomlist}
					do
						echo "	waitforiomstate -t1200 tl1sp ${iom} oosma,oos" >>${duplexscript}
						echo "	waitforiomstate -t1200 tl1sp ${iom} oosma,oos" >>${simplexspascript}
					...
					for iom in ${iomlist}
					do
						echo "	tl1 exec -v tl1sp rst-eqpt::iom-${iom}" >>${duplexscript}
						echo "	tl1 exec -v tl1sp rst-eqpt::iom-${iom}" >>${simplexspascript}
						echo "	tl1 exec -v tl1sp rst-eqpt::iom-${iom}" >>${simplexspbscript}
					done
					# wait for ioms to restore.
					for iom in ${iomlist}
					do
						echo "	waitforiomstate -t1200 tl1sp ${iom} isact,isstbyc,isstbyh,isactnbk" >>${duplexscript}
						echo "	waitforiomstate -t1200 tl1sp ${iom} isact,isstbyc,isstbyh,isactnbk" >>${simplexspascript}
						echo "	waitforiomstate -t1200 tl1sp ${iom} isact,isstbyc,isstbyh,isactnbk" >>${simplexspbscript}
					done
				done
			fi
			;;
