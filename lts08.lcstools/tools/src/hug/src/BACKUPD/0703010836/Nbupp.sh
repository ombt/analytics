...
takes precedence. \n
\n 
If the -u option used, then the user map file must have this format: \n
\n
...
EOF
	...
	then
		...
		do
			echo "	telnet exec -t120 -i -v telnetspa /bin/rm -f ${ofcprod}"
			echo "	telnet exec -t120 -i -v telnetspb /bin/rm -f ${ofcprod}"
			echo "	ftp put -t1800 -b ftpspa ${userprod} ${ofcprod}"
			echo "	ftp put -t1800 -b ftpspb ${userprod} ${ofcprod}"
		done >>$outfile
		#
...
then
	...
	#
	lcslockadm -P reload ${labid} 2>&1 | grep "P.*failed"
	if [[ $? == 0 ]]
	then
		echo
