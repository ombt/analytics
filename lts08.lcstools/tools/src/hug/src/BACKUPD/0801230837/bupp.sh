...
removeload exec {
	...
	telnet exec telnetspa find . -type d -print | cpio -pduvm /Telica/dbCurrent
	telnet exec telnetspa cp Telica_IP Telica_Hostname /Telica/dbCurrent
	telnet exec telnetspa echo '# template rc.local file' >/Telica/dbCurrent/rc.local
	telnet exec telnetspa cd /Telica
	telnet exec -i telnetspa rm -rf SAVEdbCurrent 1>/dev/null 2>&1
	#
	telnet exec telnetspa cd /Telica
	...
	telnet exec telnetspb find . -type d -print | cpio -pduvm /Telica/dbCurrent
	telnet exec telnetspb cp Telica_IP Telica_Hostname /Telica/dbCurrent
	telnet exec telnetspb echo '# template rc.local file' >/Telica/dbCurrent/rc.local
	telnet exec telnetspb cd /Telica
	telnet exec -i telnetspb rm -rf SAVEdbCurrent 1>/dev/null 2>&1
	#
	telnet exec telnetspb cd /Telica
	...
	then
		...
		then
			...
			then
				if [[ "${provisiononly}" == yes ]]
				then
					sections="initsys_${activesp} provision_${activesp}"
				else
