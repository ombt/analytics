...
usermapfile=
#
set -- `getopt ?XCfetxb:o:c:i:T:U:u:LGPIEABm:d ${*}`
if [[ ${?} -ne 0 ]]
then
	echo $usage >&2
	exit 2
...
then
	...
	do
		...
		*)
		...
		then
			echo
			echo "Errors found in user map file. Exit."
			exit 2
		fi
	done
	#
...
writeftpload () {
	...
	#
	uprintf -q -f"%s\n" type in ${LCSTOOLSDATA}/chassis2ioms where branch req ^${branch}$ and labid req ^${labid}$ and type rne ^bts$ and type rne ^tca$ and type rne debug_cpu | 
	sort -u >${iomtypes}
	#
	if [[ -s "${iomtypes}" ]]
	then
		...
		then
			...
			then
				...
				#
				uprintf -q -f"%s\n" type in ${LCSTOOLSDATA}/images where branch req ^${branch}$ and cpuload req ^${cpuload}$ and type rne ^bts$ and type rne ^tca$ and type rne debug_cpu | 
				sort -u >${iomtypes}
			fi
		else
			echo
...
EOF
		...
		*hardware*)
			...
			then
				...
				#
				# first batch of IOMS - rmv the ioms
				#
				...
				do
					...
					#
					echo "	tl1 exec -i -v tl1sp rtrv-eqpt" >>${duplexscript}
					echo "	tl1 exec -i -v tl1sp rtrv-eqpt" >>${simplexspascript}
					echo "	tl1 exec -i -v tl1sp rtrv-eqpt" >>${simplexspbscript}
					#
					for iom in ${iomlist}
					do
