...
proc logusage { logfname } {
	...
	global argv0;
	#
	logopen $env(LCSHUGLOGFILES)/$logfname logfd;
...
#
proc logerror { logfile { errormsg  "" } } {
	if {[string length $errormsg] > 0} {
		puts $errormsg;
		logmsg $logfile "ERROR: $errormsg";
	}
}
#
proc logwarning { logfile { warningmsg  "" } } {
	if {[string length $warningmsg] > 0} {
		puts $warningmsg;
		logmsg $logfile "WARNING: $warningmsg";
	}
}
