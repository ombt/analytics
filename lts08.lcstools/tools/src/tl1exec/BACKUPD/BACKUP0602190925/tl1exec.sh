...
proc usage { } {
	...
	puts "usage: [file tail $argv0] \[-\?] \[-x] \[-T] \[-V] \[-t tid] ";
	puts "	\[-m max_logon_attempts] \[-S trace level] \[-l logfile] ";
	puts "	\[-u username] \[-p passwd] ";
	puts "	\[-i ip-addr] \[-A | B] \[-L labid] scriptname";
	puts "";
	puts "where:";
	...
	puts "";
	puts "	-u username - tl1 user name";
	puts "	-p passwd - tl1 user passwd";
	puts "	-i ip-addr - IP addr of active SP";
	puts "";
	puts "	-A - use SP-A IP";
	puts "	-B - use SP-B IP";
	puts "	-L labid - labid to use, default is from variable LABID";
	puts "";
	puts "	default max_logon_attempts = $maxlogons";
	puts "	default username = $username";
	puts "	default passwd = $userpasswd";
	puts "";
	puts "if '-i' option is given, it takes precedence over any '-A',";
	puts "'-B' or '-L' option. if it is not given, then the LABID is used";
	puts "and the user is asked which SP to use if neither '-A' or '-B' was entered."
	puts "";
	puts "logging is disabled by default. if logging is enabled with a ";
	puts "'-l' cmd, all cmds which are executed will be logged in five ";
	puts "files in the home directory. the files are completed.tl1exec, ";
	puts "denied.tl1exec, inprogress.tl1exec, timedout.tl1exec and ";
	puts "misc.tl1exec. output is appended to the files. each new session ";
	puts "starts with a timestamp.";
}
#
...
if {$ipaddr == ""} {
	...
	if {![info exists obuf] || [llength obuf] == 0} {
		puts "tl1exec: no IPs found for labid $labid.";
		exit 2;
	}
	#
