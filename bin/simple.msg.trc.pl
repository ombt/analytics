...
usage: $arg0 [-?] [-h]  \\ 
        ...
        [-t | -T] [-R] \\ 
        [-P xml|pm|all] [-p] [-d] \\
        [-S yyyymmddhhmmss] [-E yyyymmddhhmmss] \\
        [-X yyyymmddhhmmss] \\
        panacim-log-file ...
...
sub date_filter
{
    ...
    {
        return FALSE if (($start_date ne "") &&
                         ($start_date gt $$ptstamp));
        return FALSE if (($end_date ne "") &&
                         ($end_date lt $$ptstamp));
        return TRUE;
    }
    elsif (($start_date ne "") &&
...
sub process_file
{
    ...
    while (my $rec = <$infh>)
    {
        ...
        #
        my $tstamp = "";
        if (date_filter($rec, \$tstamp) == TRUE)
        {
...
#
my %opts;
if (getopts('?hwWv:l:tTRP:pdS:E:X:', \%opts) != 1)
{
    usage($cmd);
    exit 2;
}
