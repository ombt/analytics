...
get_machines <- function(db_name, route)
{
    ...
    if ((db_name %in% names(db_machines_cache)) &&
        (route %in% names(db_machines_cache[[db_name]])))
    {
        return(db_machines_cache[[db_name]][[route]])
    }
    else
    {
        pg_db <- pg_open_db(db_name=db_name)
        ...
        pg_close_db(pg_db)
        db_machines_cache[[db_name]][[route]] <<- machines$machine
        return(db_machines_cache[[db_name]][[route]])
    }
}
...
get_products <- function(db_name, route)
{
    ...
    if ((db_name %in% names(db_products_cache)) &&
        (route %in% names(db_products_cache[[db_name]])))
    {
        return(db_products_cache[[db_name]][[route]])
    }
    else
    {
        pg_db <- pg_open_db(db_name=db_name)
        ...
        pg_close_db(pg_db)
        db_products_cache[[db_name]][[route]] = 
