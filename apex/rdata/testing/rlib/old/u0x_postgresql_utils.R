...
pg_load_mount_exchange_reel <- function(db, 
                                        nrows=0,
                                        query="select
                                                   ftf._filename_route,
                                                   ufd._machine_order,
...
pg_load_mount_latest_reel <- function(db, 
                                      nrows=0,
                                      query="select
                                                 ftf._filename_route,
                                                 ufd._machine_order,
...
pg_load_mount_normal_trace <- function(db, 
                                       nrows=0,
                                       query="select
                                                  ftf._filename_route,
                                                  ufd._machine_order,
...
pg_load_mount_quality_trace <- function(db, 
                                        nrows=0,
                                        query="select
                                                   ftf._filename_route,
                                                   ufd._machine_order,
...
generate_equal_or_in_clause <- function(db_column, values, equal_to=TRUE)
{
    #
    # coerce to a vector
    #
    ...
    {
        traceback()
        stop("values cannot be coerced to a vector!")
    }
    else if (length(vvals) == 0)
    {
        traceback()
        stop("null-length values parameter given!")
    }
    #
    ...
    else
    {
        if (typeof(vvals) == "character")
        {
            if (equal_to == TRUE)
                ...
                oper = " not in ( '"
            clause = paste(db_column, " not in ( '", paste(vvals,collapse="', '"), "' )", sep="")
        }
        else
        {
            if (equal_to == TRUE)
                ...
                oper = "not in ("
            clause = paste(db_column, "not in (", paste(vvals,collapse=","), ")")
        }
    }
